...
def parse_urlf_object_name(line: str) -> Optional[str]:
    prefix = "set shared profiles url-filtering"
...
def parse_description_value(line: str) -> Optional[str]:
    if " description" not in line:
        return None
    m = re.search(r"\sdescription\s+(.*)$", line)
    if not m:
        return None
...
def classify_action_line(line: str, obj_name: str) -> Optional[Tuple[str, List[str]]]:
    if " credential-enforcement allow" in line:
        m = re.search(r"\scredential-enforcement\s+allow\s+(.*)$", line)
        toks = tokenize_pan_tail(m.group(1)) if m else []
        return ("justify", toks)

    if " credential-enforcement block" in line:
        m = re.search(r"\scredential-enforcement\s+block\s+(.*)$", line)
        toks = tokenize_pan_tail(m.group(1)) if m else []
        return ("block", toks)

    obj_esc = re.escape(obj_name)

    allow_re = re.compile(rf"^set shared profiles url-filtering\s+(\"{obj_esc}\"|{obj_esc})\s+allow\s+(.*)$")
    m = allow_re.match(line)
    if m:
        return ("allow", tokenize_pan_tail(m.group(2)))

    alert_re = re.compile(rf"^set shared profiles url-filtering\s+(\"{obj_esc}\"|{obj_esc})\s+alert\s+(.*)$")
    m = alert_re.match(line)
    if m:
        return ("alert", tokenize_pan_tail(m.group(2)))

    block_re = re.compile(rf"^set shared profiles url-filtering\s+(\"{obj_esc}\"|{obj_esc})\s+block\s+(.*)$")
    m = block_re.match(line)
    if m:
        return ("block", tokenize_pan_tail(m.group(2)))
...
def main() -> int:
    ...
    try:
        ...
        pan_to_versa_path = os.path.join(base, "miscellaneous", "PAN-to-Versa-urlf-categories.txt")
        ...
        log(f"Loaded PAN->Versa mappings: {len(pan_to_versa)}")
