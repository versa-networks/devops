...
def parse_address_objects(lines: List[str], logger: logging.Logger) -> List[Dict[str, Optional[str]]]:
    ...
    obj_names: List[str] = []
    re_head = re.compile(r'^\s*set\s+shared\s+address\s+(".*?"|\S+)\s+(\S+)\s+(.*)\s*$')
    ...
    for name, lns in obj_lines.items():
        ...
        for ln in lns:
            ...
            if keyword in ("ip-netmask", "fqdn", "ip-range") and obj_type is None:
                obj_type = keyword

            if keyword == "ip-netmask":
                ip_value = rest.split()[0] if rest else None
            elif keyword == "fqdn":
                fqdn_value = rest if rest else None
            elif keyword == "ip-range":
                range_value = rest if rest else None
        ...
        if obj_type is None:
            logger.warning("Skipping object '%s' (no ip-netmask/fqdn/ip-range found).", name)
            continue
...
def main():
    ...
    has_ipnet = "ip-netmask" in full_temp_text
    has_fqdn = "fqdn" in full_temp_text
    has_range = "ip-range" in full_temp_text

    logger.info("Keyword presence: ip-netmask=%s fqdn=%s ip-range=%s", has_ipnet, has_fqdn, has_range)
