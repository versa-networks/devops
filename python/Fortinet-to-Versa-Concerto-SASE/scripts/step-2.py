...
ADDR_RE = re.compile(r'^\s*firewall\s+address\s+(?:"([^"]+)"|(\S+))\s*(.*)$')
GRP_RE  = re.compile(r'^\s*firewall\s+addrgrp\s+(?:"([^"]+)"|(\S+))\s*(.*)$')
...
def parse_object_line(line: str, kind: str) -> Optional[Tuple[str, str, str, bool]]:
    ...
    if kind == "address":
        ...
        return ("firewall address", name, rest, was_quoted)
    else:
        m = GRP_RE.match(line)
        if not m:
            return None
        ...
        name = m.group(1) if was_quoted else m.group(2)
        rest = (m.group(3) or "").strip()
        return ("firewall addrgrp", name, rest, was_quoted)
...
def find_description_quote_ranges(line: str) -> List[Tuple[int, int]]:
    ...
    while i < n:
        m = re.search(r"\bcomment\b", line[i:], flags=re.IGNORECASE)
        if not m:
            break
        j = i + m.start()
        k = i + m.end()
...
def flatten_forti_address_blocks(source_conf_path: str) -> Tuple[List[str], List[str]]:
    ...
    buf = ""
    for raw_ln in raw:
        phys = sanitize_line(raw_ln)
        if phys.rstrip().endswith("\\"):
            buf += " " + phys.rstrip()[:-1]
        else:
            buf += " " + phys
    ...
    for stripped in joined:
        ...
        if low.startswith("edit "):
            ...
            if rest_edit.startswith('"'):
                ...
                cur_name = rest_edit[1:end_q] if end_q != -1 else rest_edit[1:]
            else:
                cur_name = rest_edit.split()[0] if rest_edit.split() else rest_edit
            continue
        ...
        if cur_name is not None and low.startswith("set "):
            ...
            if in_addr:
                addr_lines.append(flat_line)
            else:
                grp_lines.append(flat_line)

    return addr_lines, grp_lines
        ...
        if cur_name is not None and low.startswith("set "):
            ...
            if in_svc:
                ...
                if keyword == "member":
                    ...
                    flat_line = 'firewall service group "{}" {} {}'.format(cur_name, keyword, value).rstrip()
                    grp_lines.append(flat_line)

    return svc_lines, grp_lines
...
def process_static_rest(rest: str) -> Tuple[bool, Optional[str], bool]:
    ...
    if re.match(r"^static\s+\[", s, flags=re.IGNORECASE):
        ...
        for tok, was_quoted in toks:
            ...
            if tok2:
                out_toks2.append(tok2)

        if not out_toks2:
            return (False, None, False)

        return (True, "static [ " + " ".join(out_toks2) + " ]", changed2)
...
def process_dynamic_filter_rest(rest: str) -> Tuple[str, Optional[str]]:
    ...
    if arg.startswith("["):
        ...
        return ("keep", f"{out_prefix} [ " + " ".join(out_toks) + " ]")
...
def group_cleanup(step2_dir: str, cleaned_grp_path: str, max_len: int) -> Dict[Tuple[str, bool], str]:
    ...
    for orig, prefix, raw_name, rest, _q in records:
        ...
        low = rest.lower()
        if low.startswith("filter") or low.startswith("dynamic filter"):
            continue
...
def update_pan_rules_with_corrected_names(pan_path: str, corrected_lines_path: str,
    ...
    print(f"\n[{kind}] Updated references in forti-rules file: {pan_path}")
    print(f"  corrected-file: {corrected_lines_path}")
    print(f"  changed lines : {changed}")
    return changed

SEC_RULES_RE = re.compile(r"\bfirewall\s+policy\b", flags=re.IGNORECASE)
...
def main() -> int:
    ...
    main_dir = resolve_main_dir(script_dir)
    ...
    with open(log_path, "w", encoding="utf-8") as log_fp:
        ...
        step1_default = os.path.join(main_dir, "step-1", "step-1_cleaned-forti-rules.txt")
        step2_pan = os.path.join(step2_dir, "step-2_cleaned-forti-rules.txt")
        ...
        user_len = input_with_timeout("Enter max length (press Enter for 63, auto in 15s): ", 15) or ""
        max_len = 63
        if user_len.strip() != "":
            try:
                v = int(user_len.strip())
                if 1 <= v <= 63:
                    max_len = v
            except ValueError:
                max_len = 63
        ...
        if forti_source is None:
            src_in2 = input_with_timeout(
                "source-forti-config.conf not found. Enter path to raw Fortinet config (or Enter to skip): ", 30
            )
            src_in2 = (src_in2 or "").strip()
            if src_in2:
                forti_source = os.path.abspath(os.path.join(cwd, src_in2)) if not os.path.isabs(src_in2) else src_in2
                if not os.path.isfile(forti_source):
                    print(f"  WARNING: file not found: {forti_source} -- address/addrgrp flattening skipped.")
                    forti_source = None
            else:
                print("  WARNING: no source config provided -- address/addrgrp flattening skipped.")

        if forti_source:
            print(f"  Source config  : {forti_source}")
            flat_addr, flat_grp = flatten_forti_address_blocks(forti_source)
            print(f"  Flattened addr lines : {len(flat_addr)}")
            print(f"  Flattened grp  lines : {len(flat_grp)}")
        ...
        if forti_source:
            ...
            print(f"  Flattened service custom lines : {len(flat_svc)}")
            print(f"  Flattened service group  lines : {len(flat_svc_grp)}")
            append_lines(step2_pan, flat_svc + flat_svc_grp)
            print(f"  Appended {len(flat_svc) + len(flat_svc_grp)} lines to: {step2_pan}")
        else:
            print("  WARNING: no source config available -- service flattening skipped.")

        extracted_addr = os.path.join(step2_dir, "step-2_extracted-address.txt")
        print("\n[Step 5] Cutting address lines: prefix 'firewall address '")
        kept, cut = extract_cut_lines(step2_pan, "firewall address ", extracted_addr)
        print(f"[Step 5] Remaining lines in step-2_cleaned-forti-rules.txt: {kept}")
        ...
        print("\n[Step 15] Updating references in step-2_cleaned-forti-rules.txt using corrected address names...")
        ...
        print("\n[Step 16] Cutting address-group lines: prefix 'firewall addrgrp '")
        kept2, cut2 = extract_cut_lines(step2_pan, "firewall addrgrp ", extracted_grp)
        print(f"[Step 16] Remaining lines in step-2_cleaned-forti-rules.txt: {kept2}")
        ...
        print("\n[Step 25] Updating references in step-2_cleaned-forti-rules.txt using corrected address-group names...")
        update_pan_rules_with_corrected_names(
            step2_pan,
            corrected_grp_file,
            kind="Address-Group",
