...
def sanitize_service_name(name: str) -> str:
    """Sanitize a Fortinet service/group name to be Versa-compatible.

    Replaces every character that is not alphanumeric or a dash with an
    underscore, preserving the original length (no collapsing of runs).

    Examples:
        "Oracle DB+Listener"  -> "Oracle_DB_Listener"
        "SAP/HANA-Port"       -> "SAP_HANA-Port"
        "MSSQL @ DB Range"    -> "MSSQL___DB_Range"
        "SSH [Admin]"         -> "SSH__Admin_"
        "DB/Services-Group"   -> "DB_Services-Group"
    """
    return re.sub(r"[^a-zA-Z0-9\-]", "_", name)


def _is_block_format(path: str) -> bool:
    """Return True if the file uses Fortinet block format (edit / set / next).

    Peeks at the first non-blank, non-comment line to decide.  Returns False
    (flat format) when the file starts with 'firewall service ...'.
    """
...
def parse_service_blocks(src_file: str) -> List[Dict]:
    """Parse a Fortinet block-format service (or service-group) config file.

    Recognises all field variants inside each edit/next block:
        protocol        – one or more protocol tokens  (TCP, UDP, IP, TCP UDP …)
        tcp-portrange   – dst[:src] port spec(s);  multiple values on one line OK
        udp-portrange   – same for UDP
        tcp-src-port    – source-only TCP port range (no destination component)
        udp-src-port    – source-only UDP port range
        comment         – free-text description string
        member          – quoted member list (service-group blocks)

    Quoted names with special characters (+, /, @, #, [], (), spaces …) are
    preserved verbatim in the returned dicts; callers apply sanitization.

    Returns a list of dicts, one per edit/next block that contains a name.
    """
    services: List[Dict] = []
    current: Dict = {}
    in_block = False
    ...
    with open(src_file, "r", encoding="utf-8", errors="replace") as fh:
        for raw in fh:
            ...
            if kw == "edit":
                in_block = True
                current = {"name": toks[1] if len(toks) > 1 else ""}

            elif kw == "next":
                if in_block and current.get("name"):
                    services.append(current)
                current = {}
                in_block = False

            elif kw == "set" and in_block and len(toks) >= 2:
                field = toks[1].lower()
                values = toks[2:]          # every token after the field keyword

                if field == "protocol":
                    # Normalise to uppercase; preserve multi-protocol e.g. "TCP UDP"
                    current["protocol"] = [v.upper() for v in values]

                elif field == "tcp-portrange":
                    # One or more dst[:src] port specs on the same line e.g. "25 587"
                    current.setdefault("tcp-portrange", []).extend(values)

                elif field == "udp-portrange":
                    current.setdefault("udp-portrange", []).extend(values)

                elif field == "tcp-src-port":
                    # Source-only TCP port spec — no destination component
                    current.setdefault("tcp-src-port", []).extend(values)

                elif field == "udp-src-port":
                    current.setdefault("udp-src-port", []).extend(values)

                elif field == "comment":
                    current["comment"] = " ".join(values)

                elif field == "member":
                    # Quoted member names already split cleanly by shlex
                    current.setdefault("member", []).extend(values)

    # Tolerate files that don't end with a "next" line
    if in_block and current.get("name"):
        services.append(current)
...
def flatten_service_config(blocks: List[Dict], kind: str = "custom") -> List[str]:
    """Convert parsed service block dicts to flat-format output lines.

    Output format (one attribute per line):
        firewall service <kind> <sanitized_name> <field> <value(s)>

    All field variants are emitted, including tcp-src-port and udp-src-port,
    so downstream scripts see the complete service definition.
    """
    lines: List[str] = []
    ...
    for svc in blocks:
        ...
        if not raw_name:
            continue
        san = sanitize_service_name(raw_name)
        prefix = f"firewall service {kind} {san}"

        # protocol
        protocols = svc.get("protocol", [])
        if protocols:
            lines.append(f"{prefix} protocol {' '.join(protocols)}")

        # tcp destination (and optional source) port ranges
        for val in svc.get("tcp-portrange", []):
            lines.append(f"{prefix} tcp-portrange {val}")

        # udp destination (and optional source) port ranges
        for val in svc.get("udp-portrange", []):
            lines.append(f"{prefix} udp-portrange {val}")

        # tcp source-only port spec
        for val in svc.get("tcp-src-port", []):
            lines.append(f"{prefix} tcp-src-port {val}")

        # udp source-only port spec
        for val in svc.get("udp-src-port", []):
            lines.append(f"{prefix} udp-src-port {val}")

        # service-group member list
        members = svc.get("member", [])
        if members:
            quoted = " ".join(f'"{sanitize_service_name(m)}"' for m in members)
            lines.append(f"{prefix} member {quoted}")

        # comment
        comment = svc.get("comment", "")
        if comment:
            lines.append(f'{prefix} comment "{comment}"')

    return lines


def extract_shared_object_names(src_file: str, object_kind_token: str) -> List[str]:
    """Extract object names from a service config file.

    Supports two input formats automatically:

    Flat format  (one attribute per line, already processed):
        firewall service custom|group  NAME  [field value ...]

    Block format  (raw Fortinet config style):
        edit "NAME"
            set protocol TCP
    ...
    In block format names are sanitized (non-alphanumeric/dash → underscore) so
    they match the convention used in the flat format and in policy references.
    """
    names = []
    with open(src_file, "r", encoding="utf-8", errors="replace") as fh:
        for raw in fh:
            ...
            if (len(toks) >= 4
                    and toks[0] == "firewall"
                    and toks[1] == "service"
                    and toks[2] == object_kind_token):
                names.append(toks[3])
                continue

            # ── block format ─────────────────────────────────────────────────
            # Every "edit" line starts a new object block; the next token is
            # the (possibly quoted) object name.
            if toks[0] == "edit" and len(toks) >= 2:
                names.append(sanitize_service_name(toks[1]))
...
def build_app_list_ids_map(rules_file: str) -> Dict[str, List[str]]:
    """Legacy fallback: parse application list definitions from a flattened rules file.

    Used only when the source config is unavailable.  The preferred path is
    build_app_list_ids_map_from_source(), which reads the raw FortiGate config
    directly and is immune to upstream parsing failures.
    """
...
def build_app_list_ids_map_from_source(source_file: str) -> Dict[str, List[str]]:
    """Parse 'config application list' blocks directly from the source FortiGate config.

    Uses cfg_depth (number of open 'config' sub-blocks) instead of absolute
    nesting depth to detect list-name 'edit' lines.  This makes the function
    immune to typos like 'nex5' (instead of 'next') that shift the absolute
    depth counter but do NOT affect cfg_depth — because 'end' still correctly
    closes the corresponding 'config' sub-block even when the preceding 'edit'
    block was never closed by a proper 'next'.

    Returns a dict mapping each list name to its ordered list of numeric AppIDs:
        {"App_Control_Standard": ["26806", "16492", "38063"],
         "App_Control_Strict":   ["33", "15832", "23813", "38990"]}
    """
    app_map: Dict[str, List[str]] = {}

    in_app_block = False
    depth        = 0   # total nesting depth (used only to detect block close)
    cfg_depth    = 0   # number of 'config' sub-blocks currently open
    current_list = ""
    ...
    with open(source_file, "r", encoding="utf-8", errors="replace") as fh:
        for raw in fh:
            ...
            # ── detect entry into 'config application list' ───────────────
            if not in_app_block:
                if (kw == "config"
                        and len(toks) >= 3
                        and toks[1].lower() == "application"
                        and toks[2].lower() == "list"):
                    in_app_block = True
                    depth     = 0
                    cfg_depth = 0
                continue
            ...
            if kw == "config":
                # A nested 'config' sub-block (e.g. 'config entries')
                cfg_depth += 1
                depth     += 1
                continue

            if kw == "edit":
                # A list-name edit is a DIRECT child of 'config application
                # list' — identified by cfg_depth == 0 (no 'config' sub-block
                # is currently open).  This is correct even when the absolute
                # depth counter is off due to upstream typos.
                if cfg_depth == 0 and len(toks) >= 2:
                    current_list = toks[1]
                    if current_list not in app_map:
                        app_map[current_list] = []
                depth += 1
                continue
            ...
            if kw == "end":
                ...
                if cfg_depth > 0:
                    cfg_depth -= 1
                if depth <= 0:
                    # Closed the top-level 'config application list' block
                    in_app_block = False
                    current_list = ""
                continue

            # Correctly-spelled 'next' reduces depth; typos like 'nex5' are
            # ignored so parsing continues past them uninterrupted.
            if kw == "next":
                depth -= 1
                continue

            # Capture 'set application <id> [id2 ...]'
            if kw == "set" and len(toks) >= 3 and toks[1].lower() == "application":
                if current_list:
                    for tok in toks[2:]:
                        if tok.isdigit():
                            app_map[current_list].append(tok)

    return app_map


def _replace_application_list_segment(line: str, ids: List[str]) -> str:
    """Replace the entire 'application-list "name1" "name2" ...' segment in *line*
    with 'application-list "id1 id2 ..."'.

    Handles any number of quoted list-name tokens after the keyword.
    Returns the original line unchanged if the marker is not found.
    """
    ...
    # Walk past all quoted tokens (and any whitespace between them)
    i = 0
    end_pos = 0
    while i < len(rest):
        c = rest[i]
        if c == '"':
            ...
            break   # non-quote, non-space → end of the application-list value section

    new_segment = f'application-list "{" ".join(ids)}"'
    return line[:idx] + new_segment + rest[end_pos:]
...
def main() -> int:
    ...
    main_dir = script_dir if os.path.basename(script_dir) != "scripts" else os.path.abspath(os.path.join(script_dir, ".."))
    ...
    if os.path.isfile(step4_service):
        # ── auto-detect format and flatten block-format files ─────────────
        if _is_block_format(step4_service):
            print(f"INFO: service file is in Fortinet block format — parsing and flattening...")
            svc_blocks = parse_service_blocks(step4_service)
            flat_lines = flatten_service_config(svc_blocks, kind="custom")
            flat_svc_path = os.path.join(step5_dir, "step-5_flattened-service.txt")
            with open(flat_svc_path, "w", encoding="utf-8") as f:
                for ln in flat_lines:
                    f.write(ln + "\n")
            print(f"OK: wrote {len(flat_lines)} flattened service config line(s) to: {flat_svc_path}")
        # ── extract names (works on both flat and block format) ────────────
    ...
    if os.path.isfile(step4_svc_group):
        # ── auto-detect format and flatten block-format files ─────────────
        if _is_block_format(step4_svc_group):
            print(f"INFO: service-group file is in Fortinet block format — parsing and flattening...")
            grp_blocks = parse_service_blocks(step4_svc_group)
            flat_grp_lines = flatten_service_config(grp_blocks, kind="group")
            flat_grp_path = os.path.join(step5_dir, "step-5_flattened-service-group.txt")
            with open(flat_grp_path, "w", encoding="utf-8") as f:
                for ln in flat_grp_lines:
                    f.write(ln + "\n")
            print(f"OK: wrote {len(flat_grp_lines)} flattened service-group config line(s) to: {flat_grp_path}")
        # ── extract names (works on both flat and block format) ────────────
    ...
    # ── locate source config for direct application-list parsing ─────────────
    # Reading from the source config (rather than the step-4 cleaned rules) is
    # immune to upstream parsing failures caused by typos like 'nex5' in the
    # config that break flattening steps and silently drop entries.
    source_config_candidates = [
        os.path.join(main_dir, "source-forti-config.conf"),
        os.path.join(main_dir, "source-config.conf"),
        os.path.join(main_dir, "forti-config.conf"),
    ]
    source_config_path = next((p for p in source_config_candidates if os.path.isfile(p)), "")

    print("INFO: scanning application list definitions...")
    if source_config_path:
        app_list_ids = build_app_list_ids_map_from_source(source_config_path)
        print(f"OK: found {len(app_list_ids)} application list(s) in source config: {source_config_path}")
        for lst_name, lst_ids in app_list_ids.items():
            print(f"  '{lst_name}': {len(lst_ids)} ID(s) -> {lst_ids}")
    else:
        print("WARNING: source config not found; falling back to rules-file application list parsing.")
        print("         Typos like 'nex5' in the source config may cause incomplete ID extraction.")
        app_list_ids = build_app_list_ids_map(rules_dst)
        print(f"OK: found {len(app_list_ids)} application list definition(s) from rules file.")
    ...
    with open(rules_dst, "r", encoding="utf-8", errors="replace") as f:
        for raw in f:
            ...
            if policy and rest and rest[0] == "application-list" and len(rest) >= 2:
                # rest[1:] may contain multiple list names when a policy references
                # more than one application-list (e.g. "App_Control_Standard"
                # "App_Control_Strict").  Collect IDs from ALL of them.
                list_names = rest[1:]
                combined_ids: List[str] = []
                combined_seen: set = set()
                for list_name in list_names:
                    for id_ in app_list_ids.get(list_name, []):
                        if id_ not in combined_seen:
                            combined_ids.append(id_)
                            combined_seen.add(id_)

                if not combined_ids:
                    policies_dropped += 1
                    print(f"  DROP: policy '{policy}' application-list(s) {list_names} "
                          f"have no app IDs (category-only or unknown) - line removed")
                    continue

                # Replace the entire 'application-list "name1" "name2" ...' segment
                # with 'application-list "id1 id2 ..."' using the dedicated helper so
                # multi-name lines are correctly collapsed into one quoted value.
                new_line = _replace_application_list_segment(line, combined_ids)
