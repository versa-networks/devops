#!/usr/bin/env python3
import os
import sys
import re
import json
import shutil
import requests
from typing import Optional, List, Dict, Tuple
from requests.packages import urllib3

urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
BASE_DIR = os.path.normpath(os.path.join(SCRIPT_DIR, ".."))
FINAL_DATA_DIR = os.path.join(BASE_DIR, "final-data")
TEMP_DIR = os.path.join(BASE_DIR, "temp")
LOG_DIR = os.path.join(BASE_DIR, "log")
MISC_DIR = os.path.join(BASE_DIR, "miscellaneous")
JSON_DIR = os.path.join(BASE_DIR, "json", "policy")
STEP6_DIR = os.path.join(BASE_DIR, "step-6")
FAILED_POLICY_JSON_DIR = os.path.join(BASE_DIR, "failed-policy-json")

CLEANED_RULES = os.path.join(FINAL_DATA_DIR, "cleaned-forti-rules.txt")
STEP6_RULES = os.path.join(STEP6_DIR, "step-6_cleaned-forti-rules.txt")
TEMP_RULES = os.path.join(TEMP_DIR, "temp-cleaned-forti-rules.txt")
GENERAL_TXT = os.path.join(TEMP_DIR, "general.txt")
ZONE_CONV = os.path.join(BASE_DIR, "zone-conversion.txt")
PREDEF_APP_CONV = os.path.join(BASE_DIR, "predef-application-conversion.txt")
PREDEF_SVC_CONV = os.path.join(BASE_DIR, "predef-service-conversion.txt")
FORTI_URLF_CAT = os.path.join(MISC_DIR, "Forti-to-Versa-urlf-categories.txt")
CUSTOM_URLF_NAMES = os.path.join(FINAL_DATA_DIR, "final-custom-urlf-profile.txt")
FINAL_URLF_PROFILE = os.path.join(FINAL_DATA_DIR, "final-urlf-profile.txt")
FINAL_SVC_FILE       = os.path.join(FINAL_DATA_DIR, "final-service.txt")
FINAL_SVC_GROUP_FILE = os.path.join(FINAL_DATA_DIR, "final-service-group.txt")
LDAP_SOURCE_INPUT = os.path.join(BASE_DIR, "ldap-source-input.txt")
LDAP_GROUPS_UUID_FILE = os.path.join(TEMP_DIR, "ldap-groups-uuid.txt")
LDAP_USERS_FILE = os.path.join(TEMP_DIR, "concerto-ldap-users.txt")
SCIM_USER_INPUT = os.path.join(BASE_DIR, "scim-source-user-input.txt")
SCIM_GROUP_INPUT = os.path.join(BASE_DIR, "scim-source-group-input.txt")
SCIM_USERS_DUMP = os.path.join(BASE_DIR, "scim-users-dump.txt")
SCIM_GROUPS_DUMP = os.path.join(BASE_DIR, "scim-groups-dump.txt")
ADDR_GROUP_CACHE = os.path.join(TEMP_DIR, "adddress-group.txt")
CUST_SVC_CACHE = os.path.join(TEMP_DIR, "cust-service.txt")
CUSTOM_URL_CAT_CACHE = os.path.join(TEMP_DIR, "custom-url-category.txt")
URLF_PROFILE_CACHE = os.path.join(TEMP_DIR, "urlf-profile.txt")
LDAP_PROFILE_CACHE = os.path.join(TEMP_DIR, "ldap-profile.txt")
SCIM_PROFILE_CACHE = os.path.join(TEMP_DIR, "scim-profile.txt")
UNRESOLVED_OBJ = os.path.join(BASE_DIR, "unresolved-objects-configuration.txt")
UNRESOLVED_URL = os.path.join(BASE_DIR, "unresolved-url-configuration.txt")
UNDEFINED_SVC = os.path.join(BASE_DIR, "undefined-service.txt")
UNRESOLVED_APP = os.path.join(BASE_DIR, "unresolved-predef-application.txt")
FORTI_VERSA_APP = os.path.join(MISC_DIR, "Forti-to-Versa-applications.txt")
UNKNOWN_URLF = os.path.join(BASE_DIR, "unresolved-urlf-profile.txt")
UNRESOLVED_SCIM_USERS = os.path.join(BASE_DIR, "unresolved-scim-users.txt")
UNRESOLVED_SCIM_GROUPS = os.path.join(BASE_DIR, "unresolved-scim-groups.txt")
POST_RESULTS = os.path.join(BASE_DIR, "post-policy-results.txt")
FAILED_POST_RESPONSES = os.path.join(BASE_DIR, "failed-policy-post-response.txt")
LOG_FILE = os.path.join(LOG_DIR, "step-8.log")

log_fh = None
policy_enabled = True


def log(msg):
    print(msg)
    if log_fh:
        log_fh.write(msg + "\n")
        log_fh.flush()


def read_general():
    result = {}
    with open(GENERAL_TXT, "r") as f:
        for line in f:
            line = line.strip()
            if ">>" in line:
                key, val = re.split(r'\s*>>\s*', line, 1)
                result[key.strip()] = val.strip()
    return result


def get_session(general):
    base_url = "https://" + general["concerto-fqdn"]
    bearer = general["bearer-token"]
    csrf = general["csrf-token"]
    cookies_str = general["cookies"]
    session = requests.Session()
    for part in cookies_str.split("; "):
        if "=" in part:
            name, val = part.split("=", 1)
            session.cookies.set(name, val)
    headers = {
        "Authorization": "Bearer " + bearer,
        "X-CSRF-Token": csrf,
        "Content-Type": "application/json"
    }
    return session, headers, base_url


def paginated_get(session, headers, base_url, url_template, window_size=100):
    all_data = []
    window = 0
    while True:
        url = base_url + url_template.format(window_number=window, window_size=window_size)
        log("GET " + url)
        resp = session.get(url, verify=False, headers=headers)
        log("Response: " + str(resp.status_code))
        if resp.status_code != 200:
            log("Error response: " + resp.text[:500])
            break
        body = resp.json()
        data = body.get("data", [])
        if data is None:
            data = []
        count = len(data)
        log("Received " + str(count) + " items in window " + str(window))
        if count == 0:
            break
        all_data.extend(data)
        if count < window_size:
            break
        window += 1
    log("Total items fetched: " + str(len(all_data)))
    return all_data


def parse_bracket_list(content):
    items = []
    i = 0
    content = content.strip()
    while i < len(content):
        if content[i] == '"':
            end = content.index('"', i + 1)
            items.append(content[i + 1:end])
            i = end + 1
        elif content[i].isspace():
            i += 1
        else:
            if content[i:i+3].upper() == 'CN=':
                end = i
                while end < len(content) and content[end] != ']' and content[end] != '"':
                    if content[end] == ' ':
                        look = end + 1
                        while look < len(content) and content[look] == ' ':
                            look += 1
                        if look >= len(content) or content[look] == ']' or content[look:look+3].upper() == 'CN=':
                            break
                    end += 1
                token = content[i:end].rstrip()
                if token:
                    items.append(token)
                i = end
            else:
                end = i
                while end < len(content) and not content[end].isspace():
                    end += 1
                items.append(content[i:end])
                i = end
    return items


def get_policy_name_and_remainder(line):
    match = re.search(r'firewall\s+policy\s+', line)
    if not match:
        return None, None
    pos = match.end()
    rest = line[pos:]
    if rest.startswith('"'):
        eq = rest.index('"', 1)
        name = rest[1:eq]
        remainder = rest[eq + 1:].strip()
    else:
        sp = rest.find(' ')
        if sp == -1:
            name = rest.strip()
            remainder = ""
        else:
            name = rest[:sp]
            remainder = rest[sp + 1:].strip()
    return name, remainder


def extract_values_from_remainder(remainder, keyword):
    kw = keyword.strip()
    idx = -1
    for m in re.finditer(r'^\s*' + re.escape(kw) + r'(?:\s|$)', remainder):
        idx = m.start()
        break
    if idx == -1:
        return None
    after = remainder[idx:].strip()
    after = after[len(kw):].strip()
    if not after:
        return None
    if after.startswith('['):
        if ']' not in after:
            return parse_bracket_list(after[1:])
        bracket_end = after.index(']')
        return parse_bracket_list(after[1:bracket_end])
    else:
        # handles multiple quoted values "A" "B" "C" and unquoted space-separated values
        return parse_bracket_list(after)


def remainder_has_keyword(remainder, keyword):
    return bool(re.search(r'^\s*' + re.escape(keyword) + r'(?:\s|$)', remainder))


def load_conversion_file(filepath, delimiter=r'\s*>>\s*'):
    result = {}
    if not os.path.exists(filepath):
        return result
    with open(filepath, "r") as f:
        for line in f:
            line = line.strip()
            if ">>" in line:
                parts = re.split(delimiter, line, 1)
                result[parts[0].strip()] = parts[1].strip()
    return result


def load_name_list_file(filepath, keyword=None):
    names = set()
    if not os.path.exists(filepath):
        return names
    with open(filepath, "r") as f:
        for line in f:
            line = line.strip()
            if not line:
                continue
            if keyword:
                match = re.search(re.escape(keyword) + r'\s+', line)
                if match:
                    rest = line[match.end():].strip()
                    if rest.startswith('"'):
                        eq = rest.index('"', 1)
                        names.add(rest[1:eq])
                    else:
                        sp = rest.find(' ')
                        if sp == -1:
                            names.add(rest)
                        else:
                            names.add(rest[:sp])
            else:
                names.add(line)
    return names


def load_colon_cache(filepath):
    result = {}
    if not os.path.exists(filepath):
        return result
    with open(filepath, "r") as f:
        for line in f:
            line = line.strip()
            if ":" in line:
                parts = line.split(":", 1)
                result[parts[0].strip()] = parts[1].strip()
    return result


def load_arrow_cache(filepath):
    result = {}
    if not os.path.exists(filepath):
        return result
    with open(filepath, "r") as f:
        for line in f:
            line = line.strip()
            if ">>" in line:
                parts = re.split(r'\s*>>\s*', line, 1)
                result[parts[0].strip()] = parts[1].strip()
    return result


def save_colon_cache(filepath, data):
    with open(filepath, "w") as f:
        for name, uid in data.items():
            f.write(name + ":" + uid + "\n")


def save_arrow_cache(filepath, data):
    with open(filepath, "w") as f:
        for name, uid in data.items():
            f.write(name + " >> " + uid + "\n")


def append_line(filepath, line):
    with open(filepath, "a") as f:
        f.write(line + "\n")


def write_unresolved_urlf(line_text):
    global policy_enabled
    policy_enabled = False
    header = (
        "╔═══════════════════════════════════════════════════════════════════════════════════════╗\n"
        "║                               !!  ATTENTION  !!                                       ║\n"
        "╠═══════════════════════════════════════════════════════════════════════════════════════╣\n"
        "║  These security URL filtering profiles are being referred to by your source           ║\n"
        "║  configuration. Either the configuration for that URL filtering profile wasn't        ║\n"
        "║  included in your source configuration file or there was an error during processing.  ║\n"
        "║  The firewall policy mentioned below were converted and transferred to Concerto in     ║\n"
        "║  DISABLED status. Please remediate the configuration and manually enable the          ║\n"
        "║  affected policies.                                                                   ║\n"
        "╚═══════════════════════════════════════════════════════════════════════════════════════╝\n"
        "\n"
    )
    if not os.path.exists(UNKNOWN_URLF):
        with open(UNKNOWN_URLF, "w") as f:
            f.write(header)
    with open(UNKNOWN_URLF, "a") as f:
        f.write(line_text + "\n")


def write_unresolved_url(line_text):
    global policy_enabled
    policy_enabled = False
    header = (
        "╔═══════════════════════════════════════════════════════════════════════════════════════╗\n"
        "║                               !!  ATTENTION  !!                                       ║\n"
        "╠═══════════════════════════════════════════════════════════════════════════════════════╣\n"
        "║  These URL categories are being referred by your source configuration, firewall       ║\n"
        "║  policies configuration. If it's meant to be custom URL categories, it wasn't         ║\n"
        "║  included in your source configuration. If it's meant to be pre-defined URL           ║\n"
        "║  categories, they're not compatible with Versa URL categories.                        ║\n"
        "║  The firewall policies affected were converted and transferred to Concerto in          ║\n"
        "║  DISABLED status. Please remediate the configuration and manually enable the          ║\n"
        "║  affected policies.                                                                   ║\n"
        "╚═══════════════════════════════════════════════════════════════════════════════════════╝\n"
        "\n"
    )
    if not os.path.exists(UNRESOLVED_URL):
        with open(UNRESOLVED_URL, "w") as f:
            f.write(header)
    with open(UNRESOLVED_URL, "a") as f:
        f.write(line_text + "\n")



def write_unresolved_scim_user(name):
    global policy_enabled
    policy_enabled = False
    header = (
        "╔═══════════════════════════════════════════════════════════════════════════════════════╗\n"
        "║                               !!  ATTENTION  !!                                       ║\n"
        "╠══════════════════════════════════════════════════════════════════════════════════════╣\n"
        "║  The users listed below could not be resolved during policy conversion. They are      ║\n"
        "║  referenced in your source firewall configuration but could not be matched against    ║\n"
        "║  the SCIM or LDAP data provided. Firewall policies referring to these users have      ║\n"
        "║  been converted and transferred to Concerto in DISABLED status. Please remediate      ║\n"
        "║  the configuration and manually enable the affected policies.                         ║\n"
        "╚══════════════════════════════════════════════════════════════════════════════════════╝\n"
        "\n"
    )
    if not os.path.exists(UNRESOLVED_SCIM_USERS):
        with open(UNRESOLVED_SCIM_USERS, "w", encoding="utf-8") as f:
            f.write(header)
    with open(UNRESOLVED_SCIM_USERS, "a", encoding="utf-8") as f:
        f.write(name + "\n")


def write_unresolved_scim_group(name):
    global policy_enabled
    policy_enabled = False
    header = (
        "╔══════════════════════════════════════════════════════════════════════════════════════╗\n"
        "║                               !!  ATTENTION  !!                                       ║\n"
        "╠══════════════════════════════════════════════════════════════════════════════════════╣\n"
        "║  The groups listed below could not be resolved during policy conversion. They are     ║\n"
        "║  referenced in your source firewall configuration but could not be matched against    ║\n"
        "║  the SCIM or LDAP data provided. Firewall policies referring to these groups have     ║\n"
        "║  been converted and transferred to Concerto in DISABLED status. Please remediate      ║\n"
        "║  the configuration and manually enable the affected policies.                         ║\n"
        "╚══════════════════════════════════════════════════════════════════════════════════════╝\n"
        "\n"
    )
    if not os.path.exists(UNRESOLVED_SCIM_GROUPS):
        with open(UNRESOLVED_SCIM_GROUPS, "w", encoding="utf-8") as f:
            f.write(header)
    with open(UNRESOLVED_SCIM_GROUPS, "a", encoding="utf-8") as f:
        f.write(name + "\n")


def write_unresolved_obj(line_text):
    global policy_enabled
    policy_enabled = False
    header = (
        "╔══════════════════════════════════════════════════════════════════════════════════════╗\n"
        "║                               !!  ATTENTION  !!                                       ║\n"
        "╠══════════════════════════════════════════════════════════════════════════════════════╣\n"
        "║  The address objects listed below could not be resolved during policy conversion.     ║\n"
        "║  They are referenced in your source firewall configuration but the object             ║\n"
        "║  configuration could not be found. Firewall policies referring to these objects have  ║\n"
        "║  been converted and transferred to Concerto in DISABLED status. Please remediate      ║\n"
        "║  the configuration and manually enable the affected policies.                         ║\n"
        "╚══════════════════════════════════════════════════════════════════════════════════════╝\n"
        "\n"
    )
    if not os.path.exists(UNRESOLVED_OBJ):
        with open(UNRESOLVED_OBJ, "w", encoding="utf-8") as f:
            f.write(header)
    with open(UNRESOLVED_OBJ, "a", encoding="utf-8") as f:
        f.write(line_text + "\n")


def fetch_address_groups(session, headers, base_url, general):
    if os.path.exists(ADDR_GROUP_CACHE):
        return load_colon_cache(ADDR_GROUP_CACHE)
    tenant = general["tenant-uuid"]
    url_tpl = "/portalapi/v1/tenants/" + tenant + "/elements/endpoint/summarize?windowSize={window_size}&nextWindowNumber={window_number}&category=ADDRESS_GROUP&ecpScope=SASE"
    data = paginated_get(session, headers, base_url, url_tpl)
    result = {}
    for item in data:
        entity = item.get("entity", item)
        name = entity.get("name", "")
        uid = entity.get("uuid", "")
        if name and uid:
            result[name] = uid
    save_colon_cache(ADDR_GROUP_CACHE, result)
    log("Fetched " + str(len(result)) + " address groups")
    return result


def fetch_custom_services(session, headers, base_url, general):
    if os.path.exists(CUST_SVC_CACHE):
        cached = load_arrow_cache(CUST_SVC_CACHE)
        if cached:
            log("Loaded " + str(len(cached)) + " custom services from cache")
            return cached
        log("WARN: cust-service.txt exists but is empty — fetching from API")
    tenant = general["tenant-uuid"]
    url_tpl = "/portalapi/v1/tenants/" + tenant + "/elements/services/summary?windowSize={window_size}&nextWindowNumber={window_number}&ecpScope=SASE&filter=all"
    data = paginated_get(session, headers, base_url, url_tpl)
    result = {}
    for item in data:
        ecp = item.get("ecpUserDefined", {})
        for svc_name, svc_list in ecp.items():
            if isinstance(svc_list, list):
                for entry in svc_list:
                    name = entry.get("name", "")
                    uid = entry.get("uuid", "")
                    if name and uid:
                        result[name] = uid
    save_arrow_cache(CUST_SVC_CACHE, result)
    log("Fetched " + str(len(result)) + " custom services")
    return result


def fetch_custom_url_categories(session, headers, base_url, general):
    if os.path.exists(CUSTOM_URL_CAT_CACHE):
        return load_arrow_cache(CUSTOM_URL_CAT_CACHE)
    tenant = general["tenant-uuid"]
    url_tpl = "/portalapi/v1/tenants/" + tenant + "/sase/settings/urlCategory/summarize?windowSize={window_size}&nextWindowNumber={window_number}&category=URL_CATEGORY&ecpScope=SASE"
    data = paginated_get(session, headers, base_url, url_tpl)
    result = {}
    for item in data:
        name = item.get("name", "")
        uid = item.get("uuid", "")
        if name and uid:
            result[name] = uid
    save_arrow_cache(CUSTOM_URL_CAT_CACHE, result)
    log("Fetched " + str(len(result)) + " custom URL categories")
    return result


def fetch_urlf_profiles(session, headers, base_url, general):
    if os.path.exists(URLF_PROFILE_CACHE):
        return load_arrow_cache(URLF_PROFILE_CACHE)
    tenant = general["tenant-uuid"]
    url_tpl = "/portalapi/v1/tenants/" + tenant + "/sase/real-time/profile/urlf/summarize?nextWindowNumber={window_number}&windowSize={window_size}"
    data = paginated_get(session, headers, base_url, url_tpl)
    result = {}
    for item in data:
        name = item.get("name", "")
        uid = item.get("uuid", "")
        if name and uid:
            result[name] = uid
    save_arrow_cache(URLF_PROFILE_CACHE, result)
    log("Fetched " + str(len(result)) + " URLF profiles")
    return result


def fetch_ldap_profiles(session, headers, base_url, general):
    if os.path.exists(LDAP_PROFILE_CACHE):
        return load_colon_cache(LDAP_PROFILE_CACHE)
    tenant = general["tenant-uuid"]
    url = base_url + "/portalapi/v1/tenants/" + tenant + "/sase/authentication/profile/summarize?nextWindowNumber=0&windowSize=100&searchKeyword="
    log("GET " + url)
    resp = session.get(url, verify=False, headers=headers)
    result = {}
    if resp.status_code == 200:
        data = resp.json().get("data", [])
        for item in data:
            name = item.get("name", "")
            uid = item.get("uuid", "")
            if name and uid:
                result[name] = uid
    save_colon_cache(LDAP_PROFILE_CACHE, result)
    log("Fetched " + str(len(result)) + " LDAP profiles")
    return result


def fetch_scim_profiles(session, headers, base_url, general):
    if os.path.exists(SCIM_PROFILE_CACHE):
        return load_colon_cache(SCIM_PROFILE_CACHE)
    tenant = general["tenant-uuid"]
    url = base_url + "/portalapi/v1/tenants/" + tenant + "/sase/scim/summarize?nextWindowNumber=0&windowSize=25"
    log("GET " + url)
    resp = session.get(url, verify=False, headers=headers)
    result = {}
    if resp.status_code == 200:
        data = resp.json().get("data", [])
        for item in data:
            name = item.get("name", "")
            uid = item.get("uuid", "")
            if name and uid:
                result[name] = uid
    save_colon_cache(SCIM_PROFILE_CACHE, result)
    return result


def get_version_control(session, headers, base_url, general):
    tenant = general["tenant-uuid"]
    url = base_url + "/portalapi/v1/tenants/" + tenant + "/sase/real-time/internet-protection/summarize?nextWindowNumber=0&windowSize=10"
    log("GET " + url)
    resp = session.get(url, verify=False, headers=headers)
    if resp.status_code == 200:
        body = resp.json()
        vc = body.get("versionControl", None)
        log("versionControl: " + str(vc))
        return vc
    return None


def convert_zone(word, zone_conv):
    if word in zone_conv:
        converted = zone_conv[word]
        if converted:
            return converted
    return word


_SQL_KEYWORDS = re.compile(
    r'\b(?:SELECT|INSERT|UPDATE|DELETE|DROP|CREATE|ALTER|TABLE|DATABASE|FROM|WHERE'
    r'|AND|OR|NOT|IN|IS|NULL|JOIN|HAVING|GROUP|ORDER|BY|LIKE|BETWEEN|EXISTS'
    r'|UNION|ALL|SET|EXEC|EXECUTE|CAST|CONVERT|DECLARE|VALUES|INTO|TOP|DISTINCT'
    r'|AS|ON|CASE|WHEN|THEN|ELSE|END|WITH|INDEX)\b',
    re.IGNORECASE
)
_SQL_SPECIAL = re.compile(r"[+;'\"\\]|--")


def sanitize_description_tags(text):
    if not text:
        return text
    text = _SQL_KEYWORDS.sub("", text)
    text = _SQL_SPECIAL.sub("", text)
    text = re.sub(r'\s{2,}', ' ', text).strip()
    return text


def extract_cn_name(dn_string):
    dn = dn_string.strip().strip('"')
    match = re.match(r'CN=([^,]+)', dn, re.IGNORECASE)
    if match:
        return match.group(1)
    return dn


def extract_domain_from_dn(dn_string):
    dn = dn_string.strip().strip('"')
    if "|" in dn:
        dn = dn.split("|")[0]
    dc_parts = re.findall(r'DC=([^,|]+)', dn, re.IGNORECASE)
    if dc_parts:
        return ".".join(dc_parts)
    return ""


def extract_username_from_object(obj_str):
    obj = obj_str.strip().strip('"')
    if re.match(r'CN=', obj, re.IGNORECASE):
        return extract_cn_name(obj)
    elif '\\' in obj:
        return obj.split('\\', 1)[1]
    elif '@' in obj:
        return obj.split('@', 1)[0]
    else:
        parts = obj.rsplit('/', 1)
        if len(parts) > 1:
            return parts[1]
        return obj


def parse_source_user_list(remainders):
    """Collect ALL values from both 'groups' and 'users' fields.
    Guards with startswith so comment lines containing these words are not scanned.
    """
    combined = []
    found_any = False
    for rem in remainders:
        stripped = rem.strip()
        if stripped == "groups" or stripped.startswith("groups "):
            vals = extract_values_from_remainder(rem, "groups")
            if vals is not None:
                found_any = True
                combined.extend(vals)
    for rem in remainders:
        stripped = rem.strip()
        if stripped == "users" or stripped.startswith("users "):
            vals = extract_values_from_remainder(rem, "users")
            if vals is not None:
                found_any = True
                combined.extend(vals)
    return combined if found_any else None


def load_ldap_source_input():
    users = []
    groups = []
    if not os.path.exists(LDAP_SOURCE_INPUT):
        return users, groups
    section = None
    with open(LDAP_SOURCE_INPUT, "r") as f:
        for line in f:
            line = line.strip()
            if line == "----BEGIN USERS----":
                section = "users"
                continue
            elif line == "----END USERS----":
                section = None
                continue
            elif line == "----BEGIN GROUPS----":
                section = "groups"
                continue
            elif line == "----END GROUPS----":
                section = None
                continue
            if not line:
                continue
            if section == "users":
                users.append(line.strip('"'))
            elif section == "groups":
                match = re.search(r'(CN=[^"]+)', line)
                if match:
                    dn = match.group(1).strip().strip('"')
                    if dn:
                        groups.append(dn)
    return users, groups


def load_ldap_groups_uuid():
    """Load ../temp/ldap-groups-uuid.txt — a JSON array of {name, object} records.
    Returns a dict keyed by group name (plain text) → object string
    (full DN concatenated with SID, e.g. 'CN=group1,OU=...|S-1-5-...').
    Lookup is first attempted case-sensitively; callers may fall back to
    case-insensitive matching using the returned dict directly.
    """
    result = {}
    if not os.path.exists(LDAP_GROUPS_UUID_FILE):
        log("LDAP groups UUID file not found: " + LDAP_GROUPS_UUID_FILE)
        return result
    try:
        with open(LDAP_GROUPS_UUID_FILE, "r", encoding="utf-8") as f:
            entries = json.load(f)
        for entry in entries:
            name = entry.get("name", "").strip()
            obj = entry.get("object", "").strip()
            if name and obj:
                result[name] = obj
        log("Loaded " + str(len(result)) + " LDAP group UUID entries from " + LDAP_GROUPS_UUID_FILE)
    except Exception as e:
        log("ERROR loading LDAP groups UUID file: " + str(e))
    return result


def lookup_group_uuid_object(cn, ldap_groups_uuid_map):
    """Return the DN|SID string for a group CN, or None if not found.
    Tries exact match first, then case-insensitive fallback.
    """
    if cn in ldap_groups_uuid_map:
        return ldap_groups_uuid_map[cn]
    cn_lower = cn.lower()
    for k, v in ldap_groups_uuid_map.items():
        if k.lower() == cn_lower:
            return v
    return None


def load_ldap_users_file():
    result = {}
    if not os.path.exists(LDAP_USERS_FILE):
        log("LDAP users file not found: " + LDAP_USERS_FILE)
        return result
    try:
        with open(LDAP_USERS_FILE, "r", encoding="utf-8") as f:
            entries = json.load(f)
        for entry in entries:
            name = entry.get("name", "").strip()
            obj = entry.get("object", "").strip()
            if obj:
                cn = extract_cn_name(obj)
                if cn:
                    result[cn] = {"name": name, "object": obj}
        log("Loaded " + str(len(result)) + " LDAP user entries from " + LDAP_USERS_FILE)
    except Exception as e:
        log("ERROR loading LDAP users file: " + str(e))
    return result


def find_ldap_match(simple_name, ldap_entries, case_sensitive=True):
    for entry in ldap_entries:
        cn = extract_cn_name(entry)
        if case_sensitive:
            if cn == simple_name:
                return entry
        else:
            if cn.lower() == simple_name.lower():
                return entry
    return None


def process_zones(remainders, zone_conv):
    source_zones = []
    dest_zones = []
    skip_source = False
    skip_dest = False
    for rem in remainders:
        stripped = rem.strip()
        # Guard: only match srcintf/dstintf when they are the leading keyword of the
        # remainder, not when they appear embedded inside a comments/description value.
        if stripped == "srcintf" or stripped.startswith("srcintf "):
            vals = extract_values_from_remainder(rem, "srcintf")
            if vals is not None:
                if len(vals) == 1 and vals[0].strip('"').strip("'").lower() in ("any", "all"):
                    skip_source = True
                else:
                    for v in vals:
                        v = v.strip('"').strip("'")
                        if v.lower() not in ("any", "all"):
                            source_zones.append(convert_zone(v, zone_conv))
        if stripped == "dstintf" or stripped.startswith("dstintf "):
            vals = extract_values_from_remainder(rem, "dstintf")
            if vals is not None:
                if len(vals) == 1 and vals[0].strip('"').strip("'").lower() in ("any", "all"):
                    skip_dest = True
                else:
                    for v in vals:
                        v = v.strip('"').strip("'")
                        if v.lower() not in ("any", "all"):
                            dest_zones.append(convert_zone(v, zone_conv))
    if skip_source:
        source_zones = []
    if skip_dest:
        dest_zones = []
    return source_zones, dest_zones


def process_addresses(remainders, full_lines, addr_groups, session, headers, base_url, general):
    global policy_enabled
    if not addr_groups:
        addr_groups = fetch_address_groups(session, headers, base_url, general)
    source_addrs = []
    dest_addrs = []
    skip_source = False
    skip_dest = False
    for i, rem in enumerate(remainders):
        src_vals = extract_values_from_remainder(rem, "srcaddr")
        if src_vals is not None:
            if len(src_vals) == 1 and src_vals[0].lower() in ("any", "all"):
                skip_source = True
            else:
                for v in src_vals:
                    if v.lower() in ("any", "all"):
                        continue
                    if v in addr_groups:
                        source_addrs.append({"uuid": addr_groups[v], "name": v})
                    else:
                        write_unresolved_obj(full_lines[i] + " << UNRESOLVED " + v)
                        log("UNRESOLVED source address: " + v)
        dst_vals = extract_values_from_remainder(rem, "dstaddr")
        if dst_vals is not None:
            if len(dst_vals) == 1 and dst_vals[0].lower() in ("any", "all"):
                skip_dest = True
            else:
                for v in dst_vals:
                    if v.lower() in ("any", "all"):
                        continue
                    if v in addr_groups:
                        dest_addrs.append({"uuid": addr_groups[v], "name": v})
                    else:
                        write_unresolved_obj(full_lines[i] + " << UNRESOLVED " + v)
                        log("UNRESOLVED destination address: " + v)
    if skip_source:
        source_addrs = []
    if skip_dest:
        dest_addrs = []
    return source_addrs, dest_addrs, addr_groups


def _load_valid_versa_app_names():
    """Load the set of valid Versa predefined application names.
    Built from both Forti-to-Versa-applications.txt and predef-application-conversion.txt.
    These are the VALUES (right-hand side) — the Versa names step-6 already resolved to.
    """
    names = set()
    for filepath in [FORTI_VERSA_APP, PREDEF_APP_CONV]:
        if not os.path.exists(filepath):
            continue
        with open(filepath, "r") as f:
            for line in f:
                line = line.strip()
                if ">>" in line:
                    versa_name = re.split(r'\s*>>\s*', line, 1)[1].strip()
                    if versa_name:
                        names.add(versa_name)
    return names


def process_applications(remainders, full_lines, policy_name=""):
    """Return (predef_apps, group_apps).

    Versa app names that are entirely uppercase (e.g. AMAZON_AWS) are
    individual predefined applications and go into:
        "application": { "predefined": [...] }

    Versa app names that are NOT entirely uppercase (e.g. Adobe-Apps) are
    predefined application groups and go into:
        "applicationGroup": { "predefined": [...] }

    The isupper() check ignores non-alpha characters (underscores, hyphens)
    so AMAZON_AWS → True, Adobe-Apps → False, as expected.
    """
    app_conv = load_conversion_file(PREDEF_APP_CONV)
    valid_versa_names = _load_valid_versa_app_names()
    predef_apps = []   # all-caps  → application.predefined
    group_apps = []    # mixed-case → applicationGroup.predefined

    def _add_app(v):
        v = v.strip()
        if not v or v.lower() in ("any", "all"):
            return
        # Already in conversion map — use mapped Versa name
        if v in app_conv and app_conv[v]:
            versa_name = app_conv[v]
        # Already a valid Versa app name (step-6 resolved it from a numeric ID)
        elif v in valid_versa_names:
            versa_name = v
        else:
            # Unknown — could not be resolved to a Versa predefined app
            # (e.g. an application-list profile name that step-6 couldn't expand)
            log("UNRESOLVED application: " + v + " in policy: " + policy_name)
            append_line(UNRESOLVED_APP, policy_name + " >> " + v)
            return

        # Route to the correct bucket based on casing of the Versa name.
        # isupper() returns True when ALL cased characters are uppercase —
        # non-alpha chars (_, -) are ignored, so AMAZON_AWS → True.
        if versa_name.isupper():
            predef_apps.append(versa_name)
        else:
            group_apps.append(versa_name)

    for i, rem in enumerate(remainders):
        # Fortinet numeric app IDs via "application" keyword
        vals = extract_values_from_remainder(rem, "application")
        if vals is not None:
            if len(vals) == 1 and vals[0].lower() in ("any", "all"):
                continue
            for v in vals:
                _add_app(v)
        # Fortinet application-list: step-6 produces
        #   application-list "AppName1 AppName2 AppName3"
        # All names space-separated inside ONE quoted string.
        vals2 = extract_values_from_remainder(rem, "application-list")
        if vals2 is not None:
            for v in vals2:
                for name in v.split():
                    _add_app(name)
    return predef_apps, group_apps


def _load_known_service_names():
    """Return the set of all custom service and service-group names that were
    successfully parsed into final-service.txt and final-service-group.txt.
    Used to distinguish "not yet pushed to Concerto" from "truly undefined".
    """
    names = set()
    for filepath, keyword in [
        (FINAL_SVC_FILE,       "firewall service custom"),
        (FINAL_SVC_GROUP_FILE, "firewall service group"),
    ]:
        if not os.path.exists(filepath):
            continue
        with open(filepath, "r") as f:
            for line in f:
                line = line.strip()
                if not line.startswith(keyword):
                    continue
                rest = line[len(keyword):].strip()
                if rest.startswith('"'):
                    eq = rest.find('"', 1)
                    names.add(rest[1:eq] if eq != -1 else rest[1:])
                else:
                    sp = rest.find(' ')
                    names.add(rest[:sp] if sp != -1 else rest)
    return names


def process_services(remainders, full_lines, cust_svc_cache, session, headers, base_url, general):
    predef_conv = load_conversion_file(PREDEF_SVC_CONV)
    predefined = []
    custom = []
    for i, rem in enumerate(remainders):
        if not (rem.strip() == "service" or rem.strip().startswith("service ")):
            continue
        vals = extract_values_from_remainder(rem, "service")
        if vals is not None:
            if len(vals) == 1 and vals[0].lower() in ("any", "all"):
                continue
            for v in vals:
                if v.lower() in ("any", "all"):
                    continue
                if v in predef_conv:
                    converted = predef_conv[v]
                    predefined.append(converted if converted else v)
                else:
                    if v in cust_svc_cache:
                        custom.append({"uuid": cust_svc_cache[v], "name": v})
                    else:
                        known_svc_names = _load_known_service_names()
                        if v in known_svc_names:
                            log("WARN: service '" + v + "' exists in final-data but not yet in Concerto cache — run convert-service.py first")
                        else:
                            append_line(UNDEFINED_SVC, full_lines[i])
                            log("UNDEFINED service: " + v)
    return predefined, custom, cust_svc_cache


def _load_forti_urlf_cat_by_name():
    """Load Forti-to-Versa URL category mapping keyed by FortiName (column 1).
    File format: ID >> FortiName >> VersaName
    """
    result = {}
    if not os.path.exists(FORTI_URLF_CAT):
        return result
    with open(FORTI_URLF_CAT, "r") as f:
        for line in f:
            line = line.strip()
            if not line or line.startswith("#") or ">>" not in line:
                continue
            parts = [p.strip() for p in re.split(r'\s*>>\s*', line)]
            if len(parts) >= 3 and parts[1] and parts[2]:
                result[parts[1]] = parts[2]   # FortiName -> VersaName
    return result


def _load_custom_urlf_names():
    """Load custom URL category names from final-custom-urlf-profile.txt.
    Fortinet native format: webfilter urlfilter <n> entry ...
    """
    names = set()
    prefix = "webfilter urlfilter"
    if not os.path.exists(CUSTOM_URLF_NAMES):
        return names
    with open(CUSTOM_URLF_NAMES, "r") as f:
        for line in f:
            line = line.strip()
            if not line.startswith(prefix):
                continue
            rest = line[len(prefix):].lstrip()
            if not rest:
                continue
            if rest.startswith('"'):
                eq = rest.find('"', 1)
                names.add(rest[1:eq] if eq != -1 else rest[1:])
            else:
                sp = rest.find(' ')
                names.add(rest[:sp] if sp != -1 else rest)
    return names


def process_url_categories(remainders, full_lines, custom_url_cache, session, headers, base_url, general):
    predef_conv = _load_forti_urlf_cat_by_name()
    custom_names = _load_custom_urlf_names()
    predefined = []
    custom_cats = []
    for i, rem in enumerate(remainders):
        if not (rem.strip() == "url-category" or rem.strip().startswith("url-category ")):
            continue
        vals = extract_values_from_remainder(rem, "url-category")
        if vals is not None:
            if len(vals) == 1 and vals[0].lower() in ("any", "all"):
                continue
            for v in vals:
                if v.lower() in ("any", "all"):
                    continue
                if v in predef_conv:
                    converted = predef_conv[v]
                    if converted:
                        predefined.append(converted)
                    else:
                        predefined.append(v)
                elif v in custom_names:
                    if not custom_url_cache:
                        custom_url_cache.update(fetch_custom_url_categories(session, headers, base_url, general))
                    if v in custom_url_cache:
                        custom_cats.append({"uuid": custom_url_cache[v], "name": v})
                    else:
                        write_unresolved_url(full_lines[i] + " << UNRESOLVED " + v)
                else:
                    write_unresolved_url(full_lines[i] + " << UNRESOLVED " + v)
                    log("UNRESOLVED URL category: " + v)
    return predefined, custom_cats, custom_url_cache


def load_urlf_profile_names():
    """Load profile names from final-urlf-profile.txt.
    Fortinet native format: webfilter profile <n> ...
    """
    names = set()
    if not os.path.exists(FINAL_URLF_PROFILE):
        return names
    prefix = "webfilter profile"
    with open(FINAL_URLF_PROFILE, "r") as f:
        for line in f:
            line = line.strip()
            if not line:
                continue
            if not line.startswith(prefix):
                continue
            rest = line[len(prefix):].lstrip()
            if not rest:
                continue
            if rest.startswith('"'):
                eq = rest.find('"', 1)
                names.add(rest[1:eq] if eq != -1 else rest[1:])
            else:
                sp = rest.find(' ')
                names.add(rest[:sp] if sp != -1 else rest)
    return names


def process_security_profiles(remainders, full_lines, session, headers, base_url, general):
    has_action_allow = False
    has_action_deny = False
    has_av = False
    has_ips = False
    has_file_blocking = False
    has_url_filtering = False
    url_filtering_name = None
    for rem in remainders:
        if remainder_has_keyword(rem, "action"):
            vals = extract_values_from_remainder(rem, "action")
            if vals:
                if vals[0].lower() == "accept":
                    has_action_allow = True
                elif vals[0].lower() == "deny":
                    has_action_deny = True
        if remainder_has_keyword(rem, "av-profile"):
            has_av = True
        if remainder_has_keyword(rem, "ips-sensor"):
            has_ips = True
        if remainder_has_keyword(rem, "webfilter-profile"):
            has_url_filtering = True
            vals = extract_values_from_remainder(rem, "webfilter-profile")
            if vals:
                url_filtering_name = vals[0].strip('"')
    set_value = {}
    has_threat_profiles = has_av or has_ips or has_file_blocking
    if has_url_filtering and url_filtering_name:
        urlf_known_names = load_urlf_profile_names()
        matched = url_filtering_name in urlf_known_names
        if matched:
            urlf_cache = fetch_urlf_profiles(session, headers, base_url, general)
            if url_filtering_name in urlf_cache:
                set_value["action"] = "security-profile"
                set_value["urlFiltering"] = {
                    "ecpUserDefinedCombo": {
                        "uuid": urlf_cache[url_filtering_name],
                        "name": url_filtering_name
                    }
                }
            else:
                for i, rem in enumerate(remainders):
                    if "webfilter-profile" in rem:
                        write_unresolved_urlf(full_lines[i] + " << UNRESOLVED " + url_filtering_name)
                        break
                log("UNRESOLVED URLF profile (not on server): " + url_filtering_name)
        else:
            for i, rem in enumerate(remainders):
                if "webfilter-profile" in rem:
                    write_unresolved_urlf(full_lines[i] + " << UNRESOLVED " + url_filtering_name)
                    break
            log("UNRESOLVED URLF profile (not in known list): " + url_filtering_name)
    if has_threat_profiles:
        set_value["action"] = "security-profile"
        if has_av:
            set_value["av"] = {"predefined": "Scan Web and Email Traffic"}
        if has_ips:
            set_value["ips"] = {"predefined": "Versa Recommended Profile"}
        if has_file_blocking:
            set_value["fileFiltering"] = {"predefined": "Versa_Corporate_Profile"}
    if not set_value:
        if has_action_allow:
            set_value = {"action": "Allow", "tcpSessionKeepAlive": False}
        elif has_action_deny:
            set_value = {"action": "Deny", "tcpSessionKeepAlive": False}
        else:
            set_value = {"action": "Allow", "tcpSessionKeepAlive": False}
    return set_value


def process_ldap_source_user(remainders, full_lines, general, session, headers, base_url):
    su_list = parse_source_user_list(remainders)
    if su_list is None:
        return None
    if len(su_list) == 1 and su_list[0].lower() in ("any", "all"):
        return None
    ldap_profile_name = general.get("ldap-profile", "")
    ldap_upn = general.get("ldap-upn", "").strip()   # empty = DN format, non-empty = UPN format
    ldap_groups_uuid_enabled = general.get("ldap-groups-uuid", "").strip().lower() == "true"
    ldap_groups_uuid_map = load_ldap_groups_uuid() if ldap_groups_uuid_enabled else {}
    if ldap_groups_uuid_enabled:
        log("LDAP groups UUID mode is ENABLED (process_ldap_source_user)")
    ldap_profiles = fetch_ldap_profiles(session, headers, base_url, general)
    ldap_profile_uuid = ldap_profiles.get(ldap_profile_name, "")
    ldap_users, ldap_groups = load_ldap_source_input()
    user_section = {
        "userMatchType": "selected",
        "profile": {
            "uuid": ldap_profile_uuid,
            "name": ldap_profile_name,
            "type": "SASE_LDAP_AUTHENTICATION",
            "subtype": "SASE_LDAP_AUTHENTICATION"
        }
    }
    users_list = []
    groups_list = []
    for obj in su_list:
        if obj.lower() in ("any", "all"):
            continue
        simple_name = extract_username_from_object(obj)
        matched_user = find_ldap_match(simple_name, ldap_users)
        matched_group = find_ldap_match(simple_name, ldap_groups)
        if matched_user:
            cn = extract_cn_name(matched_user)
            ldap_users_map = load_ldap_users_file()
            entry = ldap_users_map.get(cn)
            if entry:
                users_list.append({"name": entry["object"], "id": entry["name"]})
            else:
                log("UNRESOLVED LDAP user (not in concerto-ldap-users.txt): " + cn)
                try:
                    write_unresolved_scim_user(cn)
                except Exception as e:
                    log("WARN: could not write unresolved LDAP user: " + str(e))
                continue
        elif matched_group:
            cn = extract_cn_name(matched_group)
            if ldap_groups_uuid_enabled:
                uuid_object = lookup_group_uuid_object(cn, ldap_groups_uuid_map)
                if uuid_object:
                    log("LDAP group UUID match: " + cn + " -> " + uuid_object)
                    groups_list.append({"description": cn, "name": uuid_object, "id": cn})
                else:
                    log("LDAP group UUID not found for: " + cn + " — falling back to plain DN")
                    groups_list.append({"description": cn, "name": matched_group, "id": cn})
            else:
                groups_list.append({"description": cn, "name": matched_group, "id": cn})
        else:
            log("UNRESOLVED LDAP source-user: " + obj)
            try:
                write_unresolved_scim_user(simple_name)
            except Exception as e:
                log("WARN: could not write unresolved LDAP object: " + str(e))
            continue
    # Deduplicate by 'name' to guard against merged duplicate policy blocks
    # producing the same valid group/user multiple times in the list.
    seen_group_names = set()
    deduped_groups = []
    for g in groups_list:
        key = g.get("name", "")
        if key not in seen_group_names:
            seen_group_names.add(key)
            deduped_groups.append(g)
    groups_list = deduped_groups
    seen_user_names = set()
    deduped_users = []
    for u in users_list:
        key = u.get("name", "")
        if key not in seen_user_names:
            seen_user_names.add(key)
            deduped_users.append(u)
    users_list = deduped_users
    if users_list:
        user_section["users"] = users_list
    if groups_list:
        user_section["userGroups"] = groups_list
    if not users_list and not groups_list:
        return None
    return user_section


def load_scim_users_dump():
    """Parse Redis hgetall dump of SCIM users.
    Root-cause fix: tag_name was undefined, silently skipping every entry.
    Indexed case-insensitively by externalId and displayName.
    """
    result = {}
    if not os.path.exists(SCIM_USERS_DUMP):
        return result
    with open(SCIM_USERS_DUMP, "r", encoding="utf-8", errors="replace") as f:
        raw = f.read()
    # Split on Redis numbered-list markers to isolate each entry
    chunks = re.split(r'\n\s*\d+\)', "\n" + raw)
    for chunk in chunks:
        chunk = chunk.strip()
        # JSON value entries start with "{  (outer Redis quote + JSON open brace)
        if not (chunk.startswith('"{')):
            continue
        # Strip outer Redis quote wrapping
        if chunk.startswith('"') and chunk.endswith('"'):
            chunk = chunk[1:-1]
        # Unescape Redis-escaped JSON
        unescaped = chunk.replace('\\"', '"').replace('\\\\', '\\')
        try:
            obj = json.loads(unescaped)
        except Exception:
            continue
        user = obj.get("user", {})
        if not user:
            continue
        external_id  = user.get("externalId", "")  or ""
        username     = user.get("userName", "")     or ""
        display_name = user.get("displayName", "")  or ""
        tag_name     = user.get("tagName", "")      or ""  # was undefined — root cause
        if not username:
            continue
        record = {"userName": username, "displayName": display_name, "tagName": tag_name}
        if external_id:
            result[external_id.lower()] = record
        if display_name and display_name.lower() not in result:
            result[display_name.lower()] = record
    log("SCIM dump loaded: " + str(len(result)) + " user entries")
    return result

def load_scim_groups_dump():
    """Load scim-groups-dump.txt — a JSON array of {scimName, displayName} records.
    Returns a dict keyed by displayName (lowercase) -> {displayName, scimName}.
    """
    result = {}
    if not os.path.exists(SCIM_GROUPS_DUMP):
        log("SCIM groups dump not found: " + SCIM_GROUPS_DUMP)
        return result
    try:
        with open(SCIM_GROUPS_DUMP, "r", encoding="utf-8") as f:
            entries = json.load(f)
        for entry in entries:
            display_name = (entry.get("displayName") or "").strip()
            scim_name    = (entry.get("scimName")    or "").strip()
            if display_name:
                result[display_name.lower()] = {
                    "displayName": display_name,
                    "scimName":    scim_name
                }
        log("SCIM groups dump loaded: " + str(len(result)) + " group entries")
    except Exception as e:
        log("ERROR loading SCIM groups dump: " + str(e))
    return result


def process_scim_source_user(remainders, full_lines, general, session, headers, base_url):
    su_list = parse_source_user_list(remainders)
    if su_list is None:
        return None
    if len(su_list) == 1 and su_list[0].lower() in ("any", "all"):
        return None
    scim_profile_name = general.get("scim-profile", "")
    ldap_groups_uuid_enabled = general.get("ldap-groups-uuid", "").strip().lower() == "true"
    ldap_groups_uuid_map = load_ldap_groups_uuid() if ldap_groups_uuid_enabled else {}
    if ldap_groups_uuid_enabled:
        log("LDAP groups UUID mode is ENABLED (process_scim_source_user)")
    scim_profiles = fetch_scim_profiles(session, headers, base_url, general)
    scim_profile_uuid = scim_profiles.get(scim_profile_name, "")
    ldap_users, ldap_groups = load_ldap_source_input()
    scim_lookup = load_scim_users_dump()
    scim_groups_lookup = load_scim_groups_dump()
    user_section = {
        "userMatchType": "selected",
        "profile": {
            "uuid":    scim_profile_uuid,
            "name":    scim_profile_name,
            "type":    "SASE_SCIM_AUTHENTICATION",
            "subtype": "SASE_SCIM_AUTHENTICATION"
        }
    }
    users_list = []
    groups_list = []
    for obj in su_list:
        if obj.lower() in ("any", "all"):
            continue
        simple_name = extract_username_from_object(obj)
        matched_in_users = find_ldap_match(simple_name, ldap_users, case_sensitive=False)
        matched_in_groups = find_ldap_match(simple_name, ldap_groups, case_sensitive=False)
        if matched_in_users:
            user_simple = extract_cn_name(matched_in_users)
            scim_record = scim_lookup.get(user_simple.lower())
            if scim_record:
                users_list.append({
                    "name": scim_record["displayName"] or user_simple,
                    "id": scim_record["userName"],
                    "description": scim_record["tagName"]
                })
            else:
                try:
                    write_unresolved_scim_user(user_simple)
                except Exception as e:
                    log("WARN: could not write unresolved SCIM user: " + str(e))
                log("UNRESOLVED SCIM user: " + user_simple)
                continue
        elif matched_in_groups:
            group_simple = extract_cn_name(matched_in_groups)
            scim_group_record = scim_groups_lookup.get(group_simple.lower())
            if scim_group_record:
                groups_list.append({
                    "name":        scim_group_record["displayName"],
                    "description": scim_group_record["scimName"],
                    "id":          scim_group_record["displayName"]
                })
                log("SCIM group matched (via LDAP source): " + scim_group_record["displayName"])
            else:
                try:
                    write_unresolved_scim_group(group_simple)
                except Exception as e:
                    log("WARN: could not write unresolved SCIM group: " + str(e))
                log("UNRESOLVED SCIM group (not in scim-groups-dump): " + group_simple)
                continue
        else:
            # Not in LDAP lists — check SCIM groups dump first, then SCIM users dump
            scim_group_record = scim_groups_lookup.get(simple_name.lower())
            if scim_group_record:
                groups_list.append({
                    "name":        scim_group_record["displayName"],
                    "id":          scim_group_record["displayName"],
                    "description": scim_group_record["scimName"]
                })
                log("SCIM group matched: " + scim_group_record["displayName"])
            else:
                scim_user_record = scim_lookup.get(simple_name.lower())
                if scim_user_record:
                    users_list.append({
                        "name":        scim_user_record.get("displayName") or simple_name,
                        "id":          scim_user_record.get("userName") or simple_name,
                        "description": scim_user_record.get("tagName") or scim_profile_name
                    })
                else:
                    try:
                        write_unresolved_scim_user(simple_name)
                    except Exception as e:
                        log("WARN: could not write unresolved SCIM user: " + str(e))
                    log("UNRESOLVED SCIM user (no match): " + simple_name)
                    continue
    # Deduplicate by 'name' to guard against merged duplicate policy blocks
    # producing the same valid group/user multiple times in the list.
    seen_group_names = set()
    deduped_groups = []
    for g in groups_list:
        key = g.get("name", "")
        if key not in seen_group_names:
            seen_group_names.add(key)
            deduped_groups.append(g)
    groups_list = deduped_groups
    seen_user_names = set()
    deduped_users = []
    for u in users_list:
        key = u.get("name", "")
        if key not in seen_user_names:
            seen_user_names.add(key)
            deduped_users.append(u)
    users_list = deduped_users
    if users_list:
        user_section["users"] = users_list
    if groups_list:
        user_section["userGroups"] = groups_list
    if not users_list and not groups_list:
        return None
    return user_section


def build_policy_json(policy_name, description, tags, match_value, set_value, version_control, username):
    global policy_enabled
    policy = {
        "lastModifiedUser": username,
        "description": description,
        "attributes": {
            "match": {
                "value": match_value
            },
            "set": {
                "value": set_value
            }
        },
        "enabled": policy_enabled,
        "uncloneable": False,
        "name": policy_name,
        "version": None,
        "originId": None,
        "type": "REAL_TIME_PROTECTION",
        "subtype": "INTERNET_PROTECTION",
        "category": None,
        "federatedPath": None,
        "isDefault": None,
        "isReusable": None,
        "tags": tags if tags else [],
        "order": {
            "bottom": True
        },
        "formMode": "CREATE",
        "deploy": False,
        "versionControl": version_control
    }
    return policy


DEDUP_FIELDS = {"srcintf", "dstintf", "srcaddr", "dstaddr", "groups", "users", "service", "url-category", "application", "comments"}


def parse_bracket_tokens_raw(content):
    tokens = []
    i = 0
    content = content.strip()
    while i < len(content):
        if content[i] == ' ':
            i += 1
        elif content[i] == '"':
            try:
                end = content.index('"', i + 1)
                tokens.append(content[i:end + 1])
                i = end + 1
            except ValueError:
                tokens.append(content[i:])
                break
        else:
            j = i
            while j < len(content) and not content[j].isspace():
                j += 1
            tokens.append(content[i:j])
            i = j
    return tokens


def dedup_tokens_preserve_order(tokens):
    seen = []
    dupes = []
    result = []
    for token in tokens:
        norm = token.strip('"').strip()
        if norm not in seen:
            seen.append(norm)
            result.append(token)
        else:
            dupes.append(token)
    return result, dupes


def check_and_dedup_lines(lines):
    cleaned = []
    total_dupes = 0
    rules_affected = set()
    log("Running duplicate field check on all policy lines...")
    for line in lines:
        name, remainder = get_policy_name_and_remainder(line)
        if name is None or not remainder:
            cleaned.append(line)
            continue
        matched_field = None
        for field in DEDUP_FIELDS:
            if remainder == field or remainder.startswith(field + " "):
                matched_field = field
                break
        if matched_field is None:
            cleaned.append(line)
            continue
        after_field = remainder[len(matched_field):].strip()
        if not after_field:
            cleaned.append(line)
            continue
        if after_field.startswith("["):
            try:
                bracket_end_idx = after_field.index("]")
            except ValueError:
                cleaned.append(line)
                continue
            bracket_content = after_field[1:bracket_end_idx]
            tokens = parse_bracket_tokens_raw(bracket_content)
            deduped, dupes = dedup_tokens_preserve_order(tokens)
            if not dupes:
                cleaned.append(line)
                continue
            rules_affected.add(name)
            total_dupes += len(dupes)
            log("  DEDUP rule [" + name + "] field [" + matched_field + "]: removed " + str([d.strip('"') for d in dupes]))
            bracket_start = line.index("[")
            bracket_end = line.rindex("]")
            new_bracket = "[ " + " ".join(deduped) + " ]"
            new_line = line[:bracket_start] + new_bracket + line[bracket_end + 1:]
            cleaned.append(new_line)
        else:
            tokens = parse_bracket_tokens_raw(after_field)
            deduped, dupes = dedup_tokens_preserve_order(tokens)
            if not dupes:
                cleaned.append(line)
                continue
            rules_affected.add(name)
            total_dupes += len(dupes)
            log("  DEDUP rule [" + name + "] field [" + matched_field + "]: removed " + str([d.strip('"') for d in dupes]))
            field_start = line.find(matched_field)
            prefix = line[:field_start + len(matched_field)]
            new_line = prefix + " " + " ".join(deduped)
            cleaned.append(new_line)
    if total_dupes > 0:
        log("Dedup summary: " + str(len(rules_affected)) + " rule(s) affected, " + str(total_dupes) + " duplicate value(s) removed")
    else:
        log("Dedup check complete: no duplicates found")
    return cleaned


def group_config_by_policy(lines):
    policies = {}
    policy_order = []
    for line in lines:
        line = line.strip()
        if not line:
            continue
        name, remainder = get_policy_name_and_remainder(line)
        if name is None:
            continue
        if name not in policies:
            policies[name] = {"remainders": [], "full_lines": []}
            policy_order.append(name)
        policies[name]["remainders"].append(remainder)
        policies[name]["full_lines"].append(line)
    return policies, policy_order


def main():
    global log_fh
    os.makedirs(LOG_DIR, exist_ok=True)
    os.makedirs(TEMP_DIR, exist_ok=True)
    os.makedirs(FINAL_DATA_DIR, exist_ok=True)
    os.makedirs(FAILED_POLICY_JSON_DIR, exist_ok=True)
    log_fh = open(LOG_FILE, "w")
    log("=" * 60)
    log("Starting Fortinet policy conversion script")
    log("=" * 60)
    if not os.path.exists(CLEANED_RULES):
        if os.path.exists(STEP6_RULES):
            shutil.copy2(STEP6_RULES, CLEANED_RULES)
            log("Copied " + STEP6_RULES + " to " + CLEANED_RULES)
        else:
            log("ERROR: Neither " + CLEANED_RULES + " nor " + STEP6_RULES + " exist. Exiting.")
            log_fh.close()
            sys.exit(1)
    shutil.copy2(CLEANED_RULES, TEMP_RULES)
    log("Copied " + CLEANED_RULES + " to " + TEMP_RULES)
    with open(TEMP_RULES, "r") as f:
        all_lines = f.readlines()
    all_lines = [l.rstrip("\n\r") for l in all_lines if l.strip()]
    if not all_lines:
        log("Input file is empty. Exiting.")
        log_fh.close()
        sys.exit(0)
    all_lines = check_and_dedup_lines(all_lines)
    general = read_general()
    log("General config loaded: " + str(list(general.keys())))
    username = general.get("concerto-user", "")
    session, headers, base_url = get_session(general)
    policies, policy_order = group_config_by_policy(all_lines)
    log("Found " + str(len(policy_order)) + " policies to process")
    addr_groups = {}
    cust_svc_cache = fetch_custom_services(session, headers, base_url, general)
    log("Custom service cache: " + str(len(cust_svc_cache)) + " entries")
    if not cust_svc_cache:
        log("WARN: No custom services found in Versa cache or API.")
        if os.path.exists(FINAL_SVC_FILE):
            local_svc_names = set()
            with open(FINAL_SVC_FILE, "r") as _f:
                for _line in _f:
                    _m = re.match(r'firewall\s+service\s+custom\s+(\S+)', _line.strip())
                    if _m:
                        local_svc_names.add(_m.group(1))
            if local_svc_names:
                log("WARN: " + str(len(local_svc_names)) + " services defined in final-service.txt but none found in Versa.")
                log("WARN: Run convert-service.py first to push services, then re-run this script.")
                log("WARN: Missing: " + ", ".join(sorted(local_svc_names)))
        else:
            log("WARN: final-service.txt not found at " + FINAL_SVC_FILE)
    custom_url_cache = {}
    has_ldap = "ldap-profile" in general and general["ldap-profile"]
    has_scim = "scim-profile" in general and general["scim-profile"]
    for policy_name in policy_order:
        log("-" * 60)
        log("Processing policy: " + policy_name)
        remainders = policies[policy_name]["remainders"]
        full_lines = policies[policy_name]["full_lines"]
        description = ""
        for rem in remainders:
            vals = extract_values_from_remainder(rem, "comments")
            if vals is not None:
                description = vals[0].strip('"')
                break
        description = sanitize_description_tags(description)
        tags = []
        global policy_enabled
        policy_enabled = True
        for rem in remainders:
            if remainder_has_keyword(rem, "status"):
                vals = extract_values_from_remainder(rem, "status")
                if vals and vals[0].lower() == "disable":
                    policy_enabled = False
        zone_conv = load_conversion_file(ZONE_CONV)
        source_zones, dest_zones = process_zones(remainders, zone_conv)
        log("Source zones: " + str(source_zones))
        log("Dest zones: " + str(dest_zones))
        has_source_addr = False
        has_dest_addr = False
        for rem in remainders:
            sv = extract_values_from_remainder(rem, "srcaddr")
            if sv is not None and not (len(sv) == 1 and sv[0].lower() in ("any", "all")):
                has_source_addr = True
            dv = extract_values_from_remainder(rem, "dstaddr")
            if dv is not None and not (len(dv) == 1 and dv[0].lower() in ("any", "all")):
                has_dest_addr = True
        source_addrs = []
        dest_addrs = []
        if has_source_addr or has_dest_addr:
            if not addr_groups:
                addr_groups = fetch_address_groups(session, headers, base_url, general)
            source_addrs, dest_addrs, addr_groups = process_addresses(
                remainders, full_lines, addr_groups, session, headers, base_url, general
            )
        log("Source addresses: " + str(len(source_addrs)))
        log("Dest addresses: " + str(len(dest_addrs)))
        predef_apps, group_apps = process_applications(remainders, full_lines, policy_name)
        log("Applications (predefined/all-caps): " + str(predef_apps))
        log("Applications (group/mixed-case): " + str(group_apps))
        predef_svcs, custom_svcs, cust_svc_cache = process_services(
            remainders, full_lines, cust_svc_cache, session, headers, base_url, general
        )
        log("Predefined services: " + str(predef_svcs))
        log("Custom services: " + str(len(custom_svcs)))
        predef_url_cats, custom_url_cats, custom_url_cache = process_url_categories(
            remainders, full_lines, custom_url_cache, session, headers, base_url, general
        )
        log("Predefined URL categories: " + str(predef_url_cats))
        log("Custom URL categories: " + str(len(custom_url_cats)))
        set_value = process_security_profiles(remainders, full_lines, session, headers, base_url, general)
        log("Set value: " + str(set_value))
        user_section = None
        if has_ldap and has_scim:
            user_section = process_scim_source_user(remainders, full_lines, general, session, headers, base_url)
        elif has_scim:
            user_section = process_scim_source_user(remainders, full_lines, general, session, headers, base_url)
        elif has_ldap:
            user_section = process_ldap_source_user(remainders, full_lines, general, session, headers, base_url)
        if user_section:
            log("User section: " + str(list(user_section.keys())))
        else:
            log("No user section")
        match_value = {}
        if predef_apps or group_apps:
            app_section = {}
            if predef_apps:
                # All-caps Versa names → individual predefined applications
                app_section["application"] = {"predefined": predef_apps}
            if group_apps:
                # Mixed-case Versa names → predefined application groups
                app_section["applicationGroup"] = {"predefined": group_apps}
            match_value["application"] = app_section
        if predef_url_cats or custom_url_cats:
            url_cat = {}
            if predef_url_cats:
                url_cat["predefined"] = predef_url_cats
            if custom_url_cats:
                url_cat["ecpUserDefinedCombo"] = custom_url_cats
            match_value["urlCategory"] = url_cat
        if user_section:
            match_value["user"] = user_section
        if source_zones or dest_zones:
            zone = {"enabled": True}
            if source_zones:
                zone["source"] = source_zones
            if dest_zones:
                zone["destination"] = dest_zones
            match_value["zone"] = zone
        if source_addrs or dest_addrs:
            address = {}
            if source_addrs:
                address["source"] = {
                    "addressGroupCombo": source_addrs,
                    "addressNegate": False
                }
            if dest_addrs:
                address["destination"] = {
                    "addressGroupCombo": dest_addrs,
                    "addressNegate": False
                }
            match_value["address"] = address
        if predef_svcs or custom_svcs:
            service = {"enabled": False}
            if predef_svcs:
                service["predefined"] = predef_svcs
            if custom_svcs:
                service["ecpUserDefinedCombo"] = custom_svcs
            match_value["service"] = service
        version_control = get_version_control(session, headers, base_url, general)
        policy_json = build_policy_json(
            policy_name, description, tags,
            match_value, set_value, version_control, username
        )
        temp_json_path = os.path.join(TEMP_DIR, "post-policy-base.json")
        with open(temp_json_path, "w") as f:
            json.dump(policy_json, f, indent=4)
        log("JSON written to " + temp_json_path)
        post_url = base_url + "/portalapi/v1/tenants/" + general["tenant-uuid"] + "/sase/real-time/internet-protection"
        log("POST " + post_url)
        try:
            resp = session.post(post_url, verify=False, headers=headers, json=policy_json)
            log("Response: " + str(resp.status_code))
            log("Response body: " + resp.text[:500])
            append_line(POST_RESULTS, policy_name + " >> " + str(resp.status_code))
            if resp.status_code != 201:
                safe_name = re.sub(r'[^\w\-. ]', '_', policy_name).strip()
                failed_json_path = os.path.join(FAILED_POLICY_JSON_DIR, safe_name + ".json")
                shutil.copy2(temp_json_path, failed_json_path)
                log("Saved failed policy JSON to " + failed_json_path)
                with open(FAILED_POST_RESPONSES, "a") as fail_fh:
                    fail_fh.write("Policy: " + policy_name + " | HTTP " + str(resp.status_code) + "\n")
                    fail_fh.write(resp.text + "\n")
                    fail_fh.write("\n\n\n")
        except Exception as e:
            log("ERROR posting policy: " + str(e))
            append_line(POST_RESULTS, policy_name + " >> ERROR: " + str(e))
        log("Policy " + policy_name + " processed")
    log("=" * 60)
    log("Script completed. Processed " + str(len(policy_order)) + " policies.")
    log("=" * 60)
    log_fh.close()


if __name__ == "__main__":
    main()