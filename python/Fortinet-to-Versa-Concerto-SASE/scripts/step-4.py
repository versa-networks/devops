...
def protocol_is_valid(line: str) -> bool:
    if find_keyword_outside_quotes(line, "tcp-portrange"):
        ...
        return bool(m)
    if find_keyword_outside_quotes(line, "udp-portrange"):
        m = re.search(r"\budp-portrange\s+\d+(?:-\d+)?\b", line)
        return bool(m)
    if find_keyword_outside_quotes(line, "protocol"):
        m = re.search(r"\bprotocol\s+\S+\b", line)
        return bool(m)
    return True
    ...
    with open(log_file_path, "a", encoding="utf-8") as log_fh:
        ...
        if src_pan_rules_default.exists():
            ...
            for attempt in range(1, 4):
                user_path = input("Enter the FULL path to the source forti-rules file to copy: ").strip()
                if not user_path:
        ...
        trunc_counter = 1
        allowed_next = {"protocol", "tcp-portrange", "udp-portrange", "tcp-src-port", "udp-src-port", "comment"}
        ...
        if corrected_service.exists() and corrected_service.stat().st_size > 0:
            ...
            for cl in corr_lines:
                sp = parse_name_token_span(cl, "firewall service custom")
                if not sp:
                    continue
                _, __, raw_tok = sp
        ...
        sg_prefix = "firewall service group"
        keep_pan2: List[str] = []
        sg_lines: List[str] = []
        ...
        if corrected_sg.exists() and corrected_sg.stat().st_size > 0:
            ...
            for cl in corrected_sg.read_text(encoding="utf-8", errors="replace").splitlines():
                sp = parse_name_token_span(cl, "firewall service group")
                if not sp:
                    continue
                _, __, raw_tok = sp
        ...
        for ln in cleaned_sg_step4.read_text(encoding="utf-8", errors="replace").splitlines():
            sp2 = parse_name_token_span(ln, "firewall service group")
            if not sp2:
                continue
            ns2, ne2, _tok2 = sp2
        ...
        # =====================================================================
        # Steps 21b/21c: Correlation — prune unused service-groups and
        # service-objects BEFORE the group-expansion pass (Step-22).
        #
        # Timing rationale:
        #   Step-20 wrote sanitised service-group names into dst_pan_rules.
        #   Step-13 wrote sanitised service-object names into dst_pan_rules.
        #   Step-22 (below) will REPLACE group names in dst_pan_rules with
        #   their expanded member lists.  After that replacement, group names
        #   no longer exist in the rules file, so correlation must happen NOW
        #   while both group names AND object names are still present.
        # =====================================================================

        # ------------------------------------------------------------------
        # Helper: extract all value-tokens after a keyword on a flat line.
        # Handles both bracket form  →  keyword [ v1 v2 ... ]
        # and plain form            →  keyword v1 v2 ...
        # After sanitisation all object names are quote-free and contain no
        # spaces, so simple whitespace-split + quote-strip is sufficient.
        # ------------------------------------------------------------------
        def _svc_extract_keyword_values(line: str, keyword: str) -> List[str]:
            vals: List[str] = []
            # Bracket form: keyword [ val1 val2 ... ]
            bracket_pat = re.compile(
                rf'\b{re.escape(keyword)}\b\s+\[\s*([^\]]*)\s*\]'
            )
            for m in bracket_pat.finditer(line):
                ...
                if inside:
                    for tok in inside.split():
                        ...
                        if tok:
                            vals.append(tok)
            # Plain form: keyword val1 val2 ...  (not bracket)
            plain_pat = re.compile(rf'\b{re.escape(keyword)}\b\s+(.*)')
            for m in plain_pat.finditer(line):
                remainder = m.group(1).strip()
                if not remainder or remainder.startswith("["):
                    continue  # bracket form already handled above
        ...
        # ------------------------------------------------------------------
        # Helper: extract the object-name field from a definition line.
        # Reuses the existing parse_name_token_span(); strips stray quotes.
        # ------------------------------------------------------------------
        def _svc_extract_def_name(line: str, prefix: str) -> Optional[str]:
            sp = parse_name_token_span(line.rstrip("\n"), prefix)
            if not sp:
                return None
            _, __, raw_tok = sp
            return raw_tok.strip('"')

        # ------------------------------------------------------------------
        # Helper: collect unique object names from a list of definition lines.
        # ------------------------------------------------------------------
        def _svc_unique_names(lines: List[str], prefix: str) -> List[str]:
            seen: set = set()
            out: List[str] = []
            for ln in lines:
                nm = _svc_extract_def_name(ln, prefix)
        ...
        # ------------------------------------------------------------------
        # Helper: check if obj_name appears in the `service` field of any
        # policy rule line.
        # ------------------------------------------------------------------
        def _svc_in_rules(obj_name: str, rule_lines: List[str]) -> bool:
            for ln in rule_lines:
                if "service" not in ln:
                    continue
                if obj_name in _svc_extract_keyword_values(ln, "service"):
                    return True
            return False

        # ------------------------------------------------------------------
        # Helper: check if obj_name appears in the `member` field of any
        # service-group definition line.
        # ------------------------------------------------------------------
        def _svc_in_group_members(obj_name: str, grp_lines: List[str]) -> bool:
            for ln in grp_lines:
                if "member" not in ln:
                    continue
                if obj_name in _svc_extract_keyword_values(ln, "member"):
                    return True
            return False

        # ------------------------------------------------------------------
        # Helper: split lines into (kept, moved) sets by unused-name lookup.
        # Lines whose definition name is in unused_set go to `moved`.
        # Lines with no parseable name (e.g. comment lines) always stay kept.
        # ------------------------------------------------------------------
        def _svc_partition(
            lines: List[str], prefix: str, unused_set: set
        ) -> Tuple[List[str], List[str]]:
            kept: List[str] = []
            moved: List[str] = []
            for ln in lines:
                nm = _svc_extract_def_name(ln, prefix)
                if nm is not None and nm in unused_set:
                    moved.append(ln)
                else:
                    kept.append(ln)
            return kept, moved
        ...
        # ══════════════════════════════════════════════════════════════════
        # Step-21b: Service-GROUP correlation
        #   Rule: a service-group is USED only if its name appears in a
        #         policy rule's `service` field.  Groups referenced only by
        #         other groups (nested members) are still treated as unused —
        #         exactly mirroring how step-3 handles address-groups.
        # ══════════════════════════════════════════════════════════════════
        log("")
        log("Step-21b: Service-group correlation — pruning groups not referenced in policy rules.")

        sg_lines_corr  = cleaned_sg_step4.read_text(encoding="utf-8", errors="replace").splitlines(True)
        rules_lines_corr = dst_pan_rules.read_text(encoding="utf-8", errors="replace").splitlines(True)

        sg_all_names = _svc_unique_names(sg_lines_corr, "firewall service group")
        log(f"  Unique service-group names found: {len(sg_all_names)}")

        unused_sg_names: List[str] = []
        used_sg_names:   List[str] = []
        for nm in sg_all_names:
            if _svc_in_rules(nm, rules_lines_corr):
                used_sg_names.append(nm)
                log(f"  [USED]   service-group in policy rules: {nm}")
            else:
                unused_sg_names.append(nm)
                log(f"  [UNUSED] service-group not in any policy service field: {nm}")

        log(f"  Result — used: {len(used_sg_names)}, unused (to prune): {len(unused_sg_names)}")

        unused_sg_file = step4_dir / "step-4_unused-service-group.txt"
        if unused_sg_names:
            unused_sg_set = set(unused_sg_names)
            kept_sg_lines, moved_sg_lines = _svc_partition(
                sg_lines_corr, "firewall service group", unused_sg_set
            )
            cleaned_sg_step4.write_text("".join(kept_sg_lines), encoding="utf-8")
            unused_sg_file.write_text("".join(moved_sg_lines), encoding="utf-8")
            log(f"  Rewrote {cleaned_sg_step4.name}: {len(kept_sg_lines)} lines kept")
            log(f"  Wrote unused service-group lines ({len(moved_sg_lines)}) -> {unused_sg_file}")
        else:
            unused_sg_file.write_text("", encoding="utf-8")
            log(f"  No unused service-groups found. Created empty -> {unused_sg_file}")

        # Reload the now-pruned service-group file for the object correlation below.
        sg_lines_pruned = cleaned_sg_step4.read_text(encoding="utf-8", errors="replace").splitlines(True)

        # ══════════════════════════════════════════════════════════════════
        # Step-21c: Service-OBJECT correlation
        #   Rule: a service object is USED if its name appears in:
        #     (a) a `member` field of any KEPT (used) service-group, OR
        #     (b) a `service` field of any policy rule line.
        #   Objects referenced only by pruned (unused) groups are still
        #   treated as unused — matching step-3's address-object logic.
        # ══════════════════════════════════════════════════════════════════
        log("")
        log("Step-21c: Service-object correlation — pruning objects not referenced in any group or policy rule.")

        svc_lines_corr = cleaned_service.read_text(encoding="utf-8", errors="replace").splitlines(True)
        svc_all_names  = _svc_unique_names(svc_lines_corr, "firewall service custom")
        log(f"  Unique service-object names found: {len(svc_all_names)}")

        unused_svc_names: List[str] = []
        used_svc_names:   List[str] = []
        for nm in svc_all_names:
            in_group = _svc_in_group_members(nm, sg_lines_pruned)
            in_rules = _svc_in_rules(nm, rules_lines_corr)
            if in_group or in_rules:
                used_svc_names.append(nm)
                where = []
                if in_group: where.append("service-group member")
                if in_rules: where.append("policy rules")
                log(f"  [USED]   service object ({', '.join(where)}): {nm}")
            else:
                unused_svc_names.append(nm)
                log(f"  [UNUSED] service object not in any group member or policy: {nm}")

        log(f"  Result — used: {len(used_svc_names)}, unused (to prune): {len(unused_svc_names)}")

        unused_svc_file = step4_dir / "step-4_unused-service.txt"
        if unused_svc_names:
            unused_svc_set = set(unused_svc_names)
            kept_svc_lines, moved_svc_lines = _svc_partition(
                svc_lines_corr, "firewall service custom", unused_svc_set
            )
            cleaned_service.write_text("".join(kept_svc_lines), encoding="utf-8")
            unused_svc_file.write_text("".join(moved_svc_lines), encoding="utf-8")
            log(f"  Rewrote {cleaned_service.name}: {len(kept_svc_lines)} lines kept")
            log(f"  Wrote unused service-object lines ({len(moved_svc_lines)}) -> {unused_svc_file}")
        else:
            unused_svc_file.write_text("", encoding="utf-8")
            log(f"  No unused service objects found. Created empty -> {unused_svc_file}")

        # ──────────────────────────────────────────────────────────────────
        # End of Steps 21b/21c.  Step-22 (group expansion in rules) follows.
        # ──────────────────────────────────────────────────────────────────
        ...
        for ln in temp_lines:
            ...
            mi_list = find_keyword_outside_quotes(ln, "member")
            if not mi_list:
                continue
            mi = mi_list[0]
