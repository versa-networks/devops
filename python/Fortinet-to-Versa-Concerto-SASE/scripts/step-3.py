...
def extract_keyword_values(line: str, keyword: str) -> List[str]:
    ...
    # Non-bracket form: keyword "val1" "val2" val3 ...
    # Grab everything after the keyword and shlex-split it so that ALL
    # space-separated quoted/unquoted tokens are captured, not just the first.
    kw_pat = re.compile(rf'\b{re.escape(keyword)}\b\s+(.*)')
    for m in kw_pat.finditer(line):
...
def main() -> int:
    ...
    # --- Pass 1: groups directly referenced in policy rules ---
    used_group_set: Set[str] = set()
    for nm in group_names:
        if referenced_in_rules(nm, rules_lines):
            used_group_set.add(nm)
            logger.info(f"GROUP used in rules (direct): {nm}")

    logger.info(f"Groups directly referenced in rules: {len(used_group_set)}")

    # --- Pass 2+: transitive membership — only propagate through KEPT groups ---
    # A group is kept only if it is a member of a group that is ITSELF already kept.
    # This prevents orphaned parent groups from dragging in their children.
    iteration = 0
    while True:
        iteration += 1
        newly_added: List[str] = []
        # Restrict to lines that belong to currently-used groups only
        used_group_lines = [
            ln for ln in group_lines_original
            if extract_addr_group_name(ln) in used_group_set
        ]
        for nm in group_names:
            if nm in used_group_set:
                continue
            if referenced_in_group_static(nm, used_group_lines):
                newly_added.append(nm)
                logger.info(f"GROUP used via membership in kept group (iter {iteration}): {nm}")
        if not newly_added:
            break
        used_group_set.update(newly_added)

    logger.info(f"Groups used after transitive resolution ({iteration} iteration(s)): {len(used_group_set)}")

    unused_group_names: List[str] = [nm for nm in group_names if nm not in used_group_set]
