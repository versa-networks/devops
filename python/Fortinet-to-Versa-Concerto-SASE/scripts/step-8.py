...
STEP6_CLEANED_RULES =SCRIPT_DIR /"../step-6/step-6_cleaned-forti-rules.txt"
STEP8_CLEANED_RULES =SCRIPT_DIR /"../step-8/step-8_cleaned-forti-rules.txt"
...
FINAL_CLEANED_RULES =SCRIPT_DIR /"../final-data/cleaned-forti-rules.txt"
...
def parse_identity_tokens (line :str )->List [TokenSpan ]:
    ...
    while i <n :
        ...
        if not in_q :
            for kw in ("groups","users"):
                kl =len (kw )
                if line [i :i +kl ]==kw :
                    prev_ok =(i ==0 )or (not line [i -1 ].isalnum ()and line [i -1 ]!="_")
                    next_ok =(i +kl >=n )or (not line [i +kl ].isalnum ()and line [i +kl ]!="_")
    ...
    while i <n :
        ...
        if line [i ]=='"':
            ...
            while i <n :
                ch =line [i ]
                if esc2 :
                    esc2 =False ;i +=1 ;continue 
                if ch =="\\":
                    esc2 =True ;i +=1 ;continue 
                if ch =='"':
                    i +=1 ;break 
                i +=1 
            tok_end =i 
                ...
                while i <n and line [i ]!='"':
                    if line [i ]==" ":
                        ...
                        while look <n and line [look ]==" ":
                            look +=1 
                        if look >=n or line [look :look +3 ].upper ()=="CN=":
                            break 
                    i +=1 
                tok_end =i 
...
def write_not_found_header (path :Path )->None :
    """Write a one-time header box to the LDAP-object-not-found report file."""
    box_width =96
    inner =box_width -2 
    ...
    lines =[
        "\u2554"+ "\u2550"*inner +"\u2557",
        pad (""),
        pad ("  \u26a0  WARNING: Unresolved LDAP Objects"),
        pad (""),
        pad ("  The LDAP objects listed below could not be resolved against the reference files."),
        pad ("  All affected firewall policies will be converted with status DISABLED."),
        pad (""),
        "\u255a"+ "\u2550"*inner +"\u255d",
        "",
    ]
...
def inject_status_disable (lines :List [str ],policies_to_disable :set )->List [str ]:
    """
    Post-process out_lines: for every policy block whose name is in
    policies_to_disable, insert '    set status disable' immediately after
    the 'edit \"<name>\"' line if that directive is not already present.
    """
    result :List [str ]=[]
    i =0 
    while i <len (lines ):
        ...
        if m and m .group (1 )in policies_to_disable :
            ...
            while i <len (lines ):
                bl =lines [i ]
                if re .match (r'^\s*next\b',bl ):
                    break 
                block .append (bl )
                i +=1 
            has_disable =any (
                re .match (r'\s*set\s+status\s+disable\b',bl )for bl in block 
            )
            if not has_disable :
                result .append ("    set status disable\n")
                log (f"Injected 'set status disable' for policy '{policy_name}'")
            result .extend (block )
            if i <len (lines ):
...
def is_simple_name_token (raw_token :str )->bool :
    ...
    if is_dn_string (t ):
        return False 
    if is_ad_user_token (raw_token ):
        return False 
    if is_canonical_token (raw_token ):
        return False 
    if is_slash_login_token (raw_token ):
        return False 
    return True 
...
def process_rules_file (rules_path :Path ,dn_index :Dict [str ,str ],cn_index :Dict [str ,str ])->Counters :
    counters =Counters ()
    policies_to_disable :set =set ()
    current_policy :Optional [str ]=None 

    out_lines :List [str ]=[]
    with rules_path .open ("r",encoding ="utf-8",errors ="replace")as f :
        for line in f :
            _m_edit =re .match (r'^\s*edit\s+"([^"]+)"',line )
            if _m_edit :
                current_policy =_m_edit .group (1 )
            elif re .match (r'^\s*next\b',line ):
                current_policy =None 
            if "groups"not in line and "users"not in line :
                out_lines .append (line )
                continue 

            tokens =parse_identity_tokens (line )
            if not tokens :
                out_lines .append (line )
                continue 
                ...
                if k in dn_index :
                    ...
                    if cn_val is not None :
                        ...
                        if ck in cn_index :
                            ...
                            modified =True 
                            counters .dn_casefixed +=1 
                            log (f"Step4: DN replaced via CN fallback: {dn_content}  -->  {ref_tok}")
                        else :
                            counters .dn_not_found +=1 
                            append_not_found (LDAP_OBJECT_NOT_FOUND ,line ,t .value .strip ())
                            if current_policy :
                                policies_to_disable .add (current_policy )
                            log (f"Step4: DN NOT FOUND (CN='{cn_val}' also not in index) -> {t.value.strip()}")
                    else :
                        counters .dn_not_found +=1 
                        append_not_found (LDAP_OBJECT_NOT_FOUND ,line ,t .value .strip ())
                        if current_policy :
            ...
            for t in tokens :
                ...
                name =strip_outer_quotes (cur ).strip ()
                uk =name .casefold ()
                if uk in cn_index :
                    ref_tok =cn_index [uk ]
                    t .new =ref_tok 
                    modified =True 
                    counters .ad_replaced +=1 
                    log (f"SimpleName: replaced '{cur.strip()}' with '{ref_tok}' (CN match='{name}')")
                else :
                    counters .ad_not_found +=1 
                    append_not_found (LDAP_OBJECT_NOT_FOUND ,line ,name )
                    if current_policy :
                        policies_to_disable .add (current_policy )
                    log (f"SimpleName: name NOT FOUND -> {name}")
    ...
    if policies_to_disable :
        log (f"Injecting 'set status disable' for {len(policies_to_disable)} policy/policies: {sorted(policies_to_disable)}")
        out_lines =inject_status_disable (out_lines ,policies_to_disable )
...
def main ()->int :
    ...
    with log_path .open ("a",encoding ="utf-8",errors ="replace")as log_fp :
        ...
        write_not_found_header (LDAP_OBJECT_NOT_FOUND )
