...
def main() -> int:
    ...
    with open(log_path, "w", encoding="utf-8") as log_fp:
        ...
        prompt_msg = (
            ...
            "  Recommended maximum is 63.\n"
            "  Any object name longer than your max will be truncated and suffixed with _x (x increments).\n"
            "Enter max length (press Enter for 63): [auto] waiting 15 seconds, then using default 63."
        )
        user_len_raw = input_with_timeout(prompt_msg, 15)
        user_len = (user_len_raw or "").strip()
        max_len = 63
        if user_len != "":
            try:
                v = int(user_len)
