...
def timed_input(logger: logging.Logger, prompt_msg: str, default: str, timeout_sec: int) -> str:
    ...
    print_box(prompt_msg + f'\n(Waiting {timeout_sec}s. Default: {default})')
...
def main() -> int:
    ...
    prompt_msg = (
        f"Maximum policy name is 63 characters by default.\n"
        f"Any policy name longer than the chosen max will be truncated.\n"
