...
def appliance_from_json(json_appliance):
    ...
    device_serial = json_appliance['Hardware']['serialNo']
    device_location = json_appliance['location']
    device_hardware = json_appliance['Hardware']['model']
...
def parse_appliance_details(all_appliance_interfaces, appliance_object_list):
    ...
    for device_details in all_appliance_interfaces:
        ...
        if device_name is not None and interface_names:
            device_dict = {'Device Name': device_name, 'Device Hardware Module': device_hardware, 'Device Serial Number': device_serial, 'Device Location Info': device_location}
    ...
    for device_object in appliance_object_list:
        ...
        for device in csv_data:
            ...
            if device_object_name == device_exist_name and len(device_object) == 4:    
                device['Device Hardware Module'] = device_object[1]
...
def get_csv (csv_data):
    ...
    for entry in csv_data:
        for key in entry.keys():
            if key != "Device Name" and key != "Device Hardware Module" and key != "Device Location Info" and key != "Device Serial Number":
                interface_keys.add(key)
    interface_keys = sorted(interface_keys, key=lambda x: (x.split('_')[-1] != "Public_IP", x.split('_')))
    #Define CSV columns
    columns = ["Device Name", "Device Hardware Module", "Device Serial Number", "Device Location Info", "All Versa interfaces IPs", "All Public/Outside IPs (detected by software)"] + interface_keys
    ...
    with open(csv_file_path, "w", encoding='UTF8', newline="") as csvfile:
        ...
        for entry in csv_data:
            ...
            row["All Versa interfaces IPs"] = ", ".join(all_ips_no_subnet)
            row["All Public/Outside IPs (detected by software)"] = ", ".join(all_public_ips)
            writer.writerow(row)
