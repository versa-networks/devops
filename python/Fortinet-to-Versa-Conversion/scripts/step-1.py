...
DEFAULT_SOURCE_REL = "../step-0/step-0_cleaned-forti-rules.txt"

LOG_FILENAME = "step-1.log"

OUT_COPY_FILENAME = "step-1_cleaned-forti-rules.txt"
...
RE_SEC_RULES = re.compile(r"(?i)\bfirewall\s+policy\b")
RE_DESC_OPEN = re.compile(r'(?i)\bcomments\b\s+"')
RE_DESC_QUOTED = re.compile(r'(?i)\bcomments\b\s+"[^"]*"')
...
def main() -> int:
    ...
    default_source_path = (main_dir / "source-forti-config.conf").resolve()
