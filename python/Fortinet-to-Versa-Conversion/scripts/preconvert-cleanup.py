...
CLEANED_RULES_PATH = FINAL_DATA_DIR / "cleaned-forti-rules.txt"
...
RE_POLICY = re.compile(r'\bfirewall\s+policy\s+("([^"]+)"|(\S+))\b')
...
def set_policy_disabled_yes(lines: List[str], policy_indices: List[int]) -> None:
    ...
    for idx in policy_indices:
        if re.search(r'\bstatus\b', lines[idx]):
            old = lines[idx]
            lines[idx] = re.sub(r'(\bstatus\b\s+)\S+', r'\1disable', lines[idx])
            if lines[idx] != old:
                log(f"Policy status set to disable (existing line): idx={idx}")
            return
    ...
    if not m:
        ...
        lines.insert(first_idx + 1, indent + "status disable\n")
        log(f"Policy status set to disable (inserted fallback line) after idx={first_idx}")
        return
...
def main() -> None:
    log("----- START pre-convert object cleanup (STRICT srcaddr/dstaddr token) -----")
    ...
    for idx, line in enumerate(lines):
        ...
        if keyword not in ("srcaddr", "dstaddr"):
            continue
    ...
    if delete_indices:
        ...
        log("No lines deleted from cleaned-forti-rules.txt")

    log("----- END pre-convert object cleanup (STRICT srcaddr/dstaddr token) -----")
    print("Done.")
