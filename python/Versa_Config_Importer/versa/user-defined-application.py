"""
This module defines the data classes for representing user-defined applications.

Each user-defined application has a name, description, family, subfamily, productivity score, risk score, tags, precedence, timeout, match IPS flag, and match rules.
"""
...
class UserDefinedApplication:
    """
    A class used to represent a User Defined Application.

    Attributes
    ----------
    name : str
        the name of the user-defined application
    description : str
        a brief description of the user-defined application
    family : str
        the family the user-defined application belongs to
    subfamily : str
        the subfamily the user-defined application belongs to
    productivity : int
        the productivity score associated with the user-defined application
    risk : int
        the risk score associated with the user-defined application
    tag : List[str]
        a list of tags associated with the user-defined application
    precedence : int
        the precedence of the user-defined application
    app_timeout : int
        the timeout of the user-defined application
    app_match_ips : bool
        the match IPS flag of the user-defined application
    app_match_rules : List[MatchRule]
        a list of match rules associated with the user-defined application
    """
    ...
    def __post_init__(self):
        ...
        if self.precedence is not None and not (0 <= self.precedence <= 65535):
            raise ValueError("precedence attribute should be within the range 0 to 65535")
