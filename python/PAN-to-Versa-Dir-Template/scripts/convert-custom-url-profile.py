...
PAN_PREFIX = "set shared profiles custom-url-category"

def parse_pan_custom_urlf_objects(pan_lines: List[str], logger: logging.Logger) -> Dict[str, List[str]]:
    grouped: Dict[str, List[str]] = {}
    skipped = 0

    for raw in pan_lines:
        line = raw.strip()
        if not line:
            continue
        if not line.startswith(PAN_PREFIX):
            skipped += 1
            continue
    ...
    logger.info(f"Parsed PAN custom-url-category objects: {len(grouped)} objects")
    if skipped:
...
def extract_patterns(lines: List[str], object_name: str, logger: logging.Logger) -> List[str]:
    ...
    for line in lines:
        ...
        try:
            obj_i = toks.index(object_name)
        except ValueError:
            obj_i = 4 if len(toks) > 4 else -1
        if obj_i < 0:
            continue

        list_i = -1
        for i in range(obj_i + 1, len(toks)):
            if toks[i] == "list":
                list_i = i
                break
        if list_i < 0 or list_i + 1 >= len(toks):
            continue
        ...
        for tok in rest:
            ...
            lead, trail, core = _split_bracket_token(tok)
...
def main() -> int:
    ...
    pan_lines = read_lines(tmp_custom)
    objects = parse_pan_custom_urlf_objects(pan_lines, logger)
    if not objects:
