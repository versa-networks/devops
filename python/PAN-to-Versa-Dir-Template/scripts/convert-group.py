#!/usr/bin/env python3
"""
convert-group.py (Step-8): Convert PAN address-group objects -> Versa config markers in ../final-data/svt-temp.cfg
...
- static can be:  static target-word  OR  static [ w1 w2 ... ]
- tag can be:     tag target-word     OR  tag [ t1 t2 ... ]
...
RE_ADDR_GRP_NAME = re.compile(r'^\s*set\s+shared\s+address-group\s+(".*?"|\S+)\b')
RE_ADDR_NAME = re.compile(r'^\s*set\s+shared\s+address\s+(".*?"|\S+)\b')

RE_STATIC_LIST = re.compile(r'\bstatic\s+\[(.*?)\]\s*$', re.IGNORECASE)
RE_STATIC_SINGLE = re.compile(r'\bstatic\s+(".*?"|\S+)\s*$', re.IGNORECASE)

RE_TAG_LIST = re.compile(r'\btag\s+\[(.*?)\]\s*$', re.IGNORECASE)
RE_TAG_SINGLE = re.compile(r'\btag\s+(".*?"|\S+)\s*$', re.IGNORECASE)

RE_DESC = re.compile(r'\bdescription\s+(.+)\s*$', re.IGNORECASE)
