...
def extract_shared_object_names(src_file: str, object_kind_token: str) -> List[str]:
    ...
    object_kind_token must be 'service' or 'service-group'
    """
    names = []
...
def find_security_rules_policy_and_rest(tokens: List[str]) -> Tuple[str, List[str]]:
    """
    Locate the first occurrence of: security rules <POLICY>
...
def sanitize_description_inner_quotes_line(line: str) -> str:
    """
    Remove inner double-quotes inside the outer description "...".
    Example:
      description "blablbla "word word" blablabla";
    becomes:
      description "blablbla word word blablabla";
    Only affects lines that contain the substring: description "
    """
    key = 'description "'
    if key not in line:
        return line
