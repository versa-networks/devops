...
def extract_service_name_from_line(line: str) -> Optional[str]:
    m = re.match(r'^\s*set\s+shared\s+service\s+("([^"]+)"|\S+)\b', line)
    if not m:
        return None
...
def parse_group(group_lines: List[str], logger: logging.Logger) -> Dict[str, str]:
    out: Dict[str, str] = {}
    ...
    tag_re = re.compile(r'^\s*set\s+shared\s+service\s+("([^"]+)"|\S+)\s+tag\s+(.*)\s*$')
    for ln in group_lines:
        m = tag_re.match(ln)
        if not m:
            continue
        ...
        if rest.startswith("[") and rest.endswith("]"):
            out["tag_value"] = rest[1:-1].strip()
        else:
            out["tag_value"] = rest
        break

    desc_re = re.compile(r'^\s*set\s+shared\s+service\s+("([^"]+)"|\S+)\s+description\s+(.*)\s*$')
    for ln in group_lines:
        m = desc_re.match(ln)
        if not m:
            continue
    ...
    port_re = re.compile(
        r'^\s*set\s+shared\s+service\s+("([^"]+)"|\S+)\s+protocol\s+(tcp|udp)\s+port\s+(\d+(?:-\d+)?)\s*$'
    )
    src_re = re.compile(
        r'^\s*set\s+shared\s+service\s+("([^"]+)"|\S+)\s+protocol\s+(tcp|udp)\s+source-port\s+(\d+)\s*$'
    )

    for ln in group_lines:
        m = port_re.match(ln)
        if m:
            out["protocol"] = m.group(3)
            out["port_value"] = m.group(4)
            out["kind"] = "range-port" if "-" in out["port_value"] else "dest-port"
            return out
    ...
    logger.warning("No protocol/port or protocol/source-port line found for group. name=%s", out.get("name", ""))
    return out
...
def main() -> int:
    ...
    while True:
        ...
        parsed = parse_group(group_lines, logger)
