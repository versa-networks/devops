...
def main() -> int:
    ...
    with open(log_path, "w", encoding="utf-8") as log_fp:
        ...
        user_len_raw = input_with_timeout("Enter max length (press Enter for 63, auto-continue in 15s): ", 15)
        if user_len_raw is None:
            print("[Step 3] No input received within 15s; using default 63.")
            user_len = ""
        else:
            user_len = user_len_raw.strip()
        max_len = 63
        if user_len != "":
            try:
                v = int(user_len)
                if 1 <= v <= 63:
                    ...
                    print(f"[Step 3] Value {v} out of range (1-63); using default 63.")
            except ValueError:
                print(f"[Step 3] Invalid integer '{user_len}'; using default 63.")
