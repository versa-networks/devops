...
def get_template_name(template_file_name: str) -> Optional[str]:
    ...
    try:
        ...
        print(f"Error: File {template_file_name} not found.")
    except Exception as e:
        print(f"Error: Unable to parse file {template_file_name}. Details: {e}")
    return None
...
def open_output_files(args: Namespace) -> Tuple[str, TextIO, str, TextIO]:
    """
    ...
    Returns:
        Tuple[str, TextIO, TextIO, TextIO]: Tuple containing the output file names and file objects.
    """
    ...
    return pan_config_file_name, out_fh, csv_out_fh, log_fh
...
def populate_interfaces_networks_zones_map(versa_intf_zone_map: Dict[str, List[str]], v_logger: logging.Logger, versa_cfg: Any, args: Any) -> Tuple[Dict[str, List[str]], logging.Logger, Any, Any]:
    """
    ...
    return versa_intf_zone_map, v_logger, versa_cfg, args
...
def load_shared_config(shared_xml, shared_tnt, v_logger,
                        predef_app_map: Dict, predef_subfamilies_map: Dict) -> None:
...
def unused(xml_root,tnt_xlate_map,versa_cfg,csv_out_fh,shared_tnt,predef_app_map,predef_url_categories_map):
    # Define header row for CSV file
    header_row = "#Tenant,SerialNum,RuleName,SrcZone,SrcAddr,DstZone,DstAddr,Schedule,Services,Service Groups,Application,URL Category,User,Action"

    # Print header row to CSV file
    print(f"{header_row}", file=csv_out_fh)

    # Define delimiter for CSV file
    delimiter = ","
    ...
    for v in vsys_list:
        ...
        for r in rules:
            ...
            rname = orig_rname.replace(" ", "_")
            xrule = cur_ngfw.get_rule(rname)

            # Print tenant
            print(f"{src_tnt}{delimiter}", end="", file=csv_out_fh)

            # Print serial number
            print(f"{count}{delimiter}", end="", file=csv_out_fh)

            # Print rule name
            print(f"{orig_rname}{delimiter}", end="", file=csv_out_fh)

            # Print source zones
            src_zones = [sz.text for sz in r.findall("./from/member")]
            print(f"{' '.join(src_zones)}{delimiter}", end="", file=csv_out_fh)

            # Print source addresses
            src_addresses = [sa.text for sa in r.findall("./source/member")]
            print(f"{' '.join(src_addresses)}{delimiter}", end="", file=csv_out_fh)

            # Print destination zones
            dst_zones = [dz.text for dz in r.findall("./to/member")]
            print(f"{' '.join(dst_zones)}{delimiter}", end="", file=csv_out_fh)

            # Print destination addresses
            dst_addresses = [da.text for da in r.findall("./destination/member")]
            print(f"{' '.join(dst_addresses)}{delimiter}", end="", file=csv_out_fh)

            # Print schedule
            schedule = r.find("./schedule")
            if schedule is not None:
                print(f"{schedule.text.strip()}{delimiter}", end="", file=csv_out_fh)
            else:
                print(f"{delimiter}", end="", file=csv_out_fh)

            # Print services
            services = [svc.text for svc in r.findall("./service/member")]
            print(f"{' '.join(services)}{delimiter}", end="", file=csv_out_fh)

            # Print service groups
            service_groups = [sg.text for sg in r.findall("./service-group/member")]
            print(f"{' '.join(service_groups)}{delimiter}", end="", file=csv_out_fh)

            # Print application
            applications = [app.text for app in r.findall("./application/member")]
            print(f"{' '.join(applications)}{delimiter}", end="", file=csv_out_fh)

            # Print URL category
            url_categories = [uc.text for uc in r.findall("./category/member")]
            print(f"{' '.join(url_categories)}{delimiter}", end="", file=csv_out_fh)

            # Print source user
            users = r.findall("./source-user/member")
            if len(users) > 0:
                user_list = [u.text for u in users]
                print(f'"{delimiter.join(user_list)}"{delimiter}', end="", file=csv_out_fh)
            else:
                print(f"{delimiter}", end="", file=csv_out_fh)
            ...
            action = r.find("./action")
            if action is not None:
                print(f"{action.text.strip()}{delimiter}", end="", file=csv_out_fh)
            else:
                print(f"{delimiter}", end="", file=csv_out_fh)
            ...
            # Print tenant
            print(f"{tnt}{delimiter}", end="", file=csv_out_fh)

            # Print serial number
            print(f"{count}.1{delimiter}", end="", file=csv_out_fh)

            # Print rule name
            print(f"{rname}{delimiter}", end="", file=csv_out_fh)

            # Print source zones
            src_zones = [zone for zone in xrule.get_src_zone_map()]
            print(f"{' '.join(src_zones)}{delimiter}", end="", file=csv_out_fh)

            # Print source addresses
            src_addresses = list(xrule.get_src_addr_map().keys()) + list(xrule.get_src_addr_grp_map().keys())
            if len(src_addresses) > 0:
                print(f"address-list: [{delimiter} {' '.join(src_addresses)} ]{delimiter}", end="", file=csv_out_fh)
            else:
                print(f"{delimiter}", end="", file=csv_out_fh)

            # Print destination zones
            dst_zones = [zone for zone in xrule.get_dst_zone_map()]
            print(f"{' '.join(dst_zones)}{delimiter}", end="", file=csv_out_fh)

            # Print destination addresses
            dst_addresses = list(xrule.get_dst_addr_map().keys()) + list(xrule.get_dst_addr_grp_map().keys())
            if len(dst_addresses) > 0:
                print(f"address-list: [{delimiter} {' '.join(dst_addresses)} ]{delimiter}", end="", file=csv_out_fh)
            else:
                print(f"{delimiter}", end="", file=csv_out_fh)

            # Print schedule
            if xrule.get_schedule() is not None:
                print(f"{xrule.get_schedule()}{delimiter}", end="", file=csv_out_fh)
            else:
                print(f"{delimiter}", end="", file=csv_out_fh)

            # Print services
            services = list(xrule.get_service_map().keys())
            print(f"{' '.join(services)}{delimiter}{delimiter}", end="", file=csv_out_fh)

            # Print application
            applications = list(xrule.get_application_map().keys())
            if len(applications) > 0:
                ...
                for app in applications:
                    if app in list(cur_tnt.application_map.keys()) or (
                        shared_tnt is not None and app in list(shared_tnt.application_map.keys())
                    ):
                        user_def_app_list.append(app)
                    elif app in list(cur_tnt.application_group_map.keys()) or (
                        shared_tnt is not None and app in list(shared_tnt.application_group_map.keys())
                    ):
                        user_def_app_grp_list.append(app)
                    elif app in list(predef_app_map):
                        predef_app_list.append(app)

                if len(predef_app_list) > 0:
                    print(f"predefined-application-list: [{delimiter} {' '.join(predef_app_list)} ]{delimiter}", end="", file=csv_out_fh)
                if len(user_def_app_list) > 0:
                    print(f"user-defined-application-list: [{delimiter} {' '.join(user_def_app_list)} ]{delimiter}", end="", file=csv_out_fh)
                if len(user_def_app_grp_list) > 0:
                    print(f"user-defined-application-group-list: [{delimiter} {' '.join(user_def_app_grp_list)} ]{delimiter}", end="", file=csv_out_fh)
            else:
                print(f"{delimiter}", end="", file=csv_out_fh)

            # Print URL category
            url_categories = list(xrule.get_url_category_map().keys())
            if len(url_categories) > 0:
                ...
                for uc in url_categories:
                    ...
                    if uc in list(cur_tnt.url_category_map.keys()) or (
                        shared_tnt is not None and uc in list(shared_tnt.url_category_map.keys())
                    ):
                        user_def_uc_list.append(uc)
                    elif uc_convert in list(predef_url_categories_map):
                        predef_uc_list.append(uc_convert)

                if len(predef_uc_list) > 0:
                    print(f"predefined [{delimiter} {' '.join(predef_uc_list)} ]{delimiter}", end="", file=csv_out_fh)

                if len(user_def_uc_list) > 0:
                    print(f"user-defined [{delimiter} {' '.join(user_def_uc_list)} ]{delimiter}", end="", file=csv_out_fh)
            else:
                print(f"{delimiter}", end="", file=csv_out_fh)
            ...
            if xrule.get_action() is not None:
                print(f"{xrule.get_action_string()}{delimiter}", end="", file=csv_out_fh)
            else:
                print(f"{delimiter}", end="", file=csv_out_fh)
...
def main(args_list: list) -> bool:
    ...
    print("Opening predefined files...")
    ...
    pan_config_file_name, out_fh, csv_out_fh, log_fh = open_output_files(args)
    ...
    versa_intf_zone_map, v_logger, versa_cfg, args = populate_interfaces_networks_zones_map(versa_intf_zone_map, v_logger, versa_cfg, args)
    ...
    load_shared_config(shared_xml, shared_tnt, v_logger, predef_app_map, predef_subfamilies_map)
    ...
    versa_cfg.write_config(tnt_xlate_map, args.template_name, args.device_name, out_fh, log_fh)
