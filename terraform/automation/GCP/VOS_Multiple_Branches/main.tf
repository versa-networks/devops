...
resource "google_compute_address" "pub_ip_mgmt" {
  count = var.vm_count
  name  = "pub-ip-mgmt-vos-${count.index}-${random_id.randomId.hex}"
}

# Create Public IP for WAN Interface for VOS instance
resource "google_compute_address" "pub_ip_wan" {
  count = var.vm_count
  name  = "pub-ip-wan-vos-${count.index}-${random_id.randomId.hex}"
}

# Create VOS Instance
resource "google_compute_instance" "vm_vos" {
  count                     = var.vm_count
  name                      = "vos-${count.index}-${random_id.randomId.hex}"
  zone                      = var.zone
  ...
  network_interface {
    ...
    access_config {
      nat_ip = google_compute_address.pub_ip_mgmt[count.index].address
    }
  }
  ...
  network_interface {
    ...
    access_config {
      nat_ip = google_compute_address.pub_ip_wan[count.index].address
    }
  }
