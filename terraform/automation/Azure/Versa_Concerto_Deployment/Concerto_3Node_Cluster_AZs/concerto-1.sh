...
hostnamectl set-hostname $Hostname_Concerto_1 >> $log_path 2>&1
hostname $Hostname_Concerto_1 >> $log_path 2>&1
...
echo "Updating clusterconfig.json file to configure Concerto in cluster mode.." >> $log_path
cat > /var/versa/ecp/share/config/clusterconfig.json << EOF
{
    "version": 4,
    "node_ip_list": [
        {
            "$Concerto_1_mgmt_ip": {
                ...
                "password": "versa123",
                "type": "primary",
                "hostname": "$Hostname_Concerto_1",
                "public_ip": "$Concerto_1_mgmt_ip"
            }
        },
        {
            "$Concerto_2_mgmt_ip": {
                ...
                "password": "versa123",
                "type": "primary",
                "hostname": "$Hostname_Concerto_2",
                "public_ip": "$Concerto_2_mgmt_ip"
            }
        },
        {
            "$Concerto_3_mgmt_ip": {
                ...
                "password": "versa123",
                "type": "primary",
                "hostname": "$Hostname_Concerto_3",
                "public_ip": "$Concerto_3_mgmt_ip"
            }
        }
    ],
    "core_replicas": 3
...
EOF
echo -e "Got below data from clusterconfig.json file:\n `cat /var/versa/ecp/share/config/clusterconfig.json`" >> $log_path
sudo chown versa:versa /var/versa/ecp/share/config/clusterconfig.json

echo -e "Adding script to deploy concerto in cluster mode." >> $log_path
...
echo "Executing the deploy script in non interactive mode.." >> $log_path
sudo /opt/versa/ecp/scripts/swarm/deploy.py -clusterfile /var/versa/ecp/share/config/clusterconfig.json -mode non-interactive -init True >> $log_path 2>&1
echo "$(date) FIN"
EOF
sudo chown versa:versa /etc/concerto-cloud-init.sh
sudo chmod 0755 /etc/concerto-cloud-init.sh

echo -e "Creating cronjob entry to run the deploy script after 10 mins from boot." >> $log_path
at now +10 min -f /etc/concerto-cloud-init.sh >> $log_path 2>&1
