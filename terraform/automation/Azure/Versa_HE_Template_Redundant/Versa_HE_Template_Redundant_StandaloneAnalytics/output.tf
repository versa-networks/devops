...
output "Versa_Analytics-1_MGMT_IP" {
  value = azurerm_network_interface.van_nic_1[0].private_ip_address
}

output "Versa_Analytics-1_Public_IP" {
  value = data.azurerm_public_ip.van_pub_ip[0].ip_address
}

output "Versa_Analytics-1_CLI_sshCommand" {
  value = "ssh -i id_rsa versa@${data.azurerm_public_ip.van_pub_ip[0].ip_address}"
}

output "Versa_Analytics-1_UI_Login" {
  value = "http://${data.azurerm_public_ip.van_pub_ip[0].ip_address}:8080\n"
}
...
output "Versa_Analytics-2_MGMT_IP" {
  value = azurerm_network_interface.van_nic_1[1].private_ip_address
}

output "Versa_Analytics-2_Public_IP" {
  value = data.azurerm_public_ip.van_pub_ip[1].ip_address
}

output "Versa_Analytics-2_CLI_sshCommand" {
  value = "ssh -i id_rsa versa@${data.azurerm_public_ip.van_pub_ip[1].ip_address}"
}

output "Versa_Analytics-2_UI_Login" {
  value = "http://${data.azurerm_public_ip.van_pub_ip[1].ip_address}:8080\n"
}